...
def main():
    ...
    print(
        f"   • Basic hook test: {'✅' if basic_result and basic_result['success'] else '❌'}"
    )
    print(
        f"   • Subagent hook test: {'✅' if subagent_result and subagent_result['success'] else '❌'}"
    )

    all_passed = (
        config_ok
        and (basic_result and basic_result["success"])
        and (subagent_result and subagent_result["success"])
    )
