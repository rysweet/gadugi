...
try:
    from core import (
        PRBacklogManager, PRAssessment, PRStatus, ReadinessCriteria, 
        BacklogMetrics, GadugiError
    )
...
class TestPRBacklogManager:
    ...
    def test_process_single_pr_success(self, pr_backlog_manager, sample_pr_data, mock_github_ops):
        ...
        pr_backlog_manager._evaluate_readiness_criteria = Mock(return_value={
    ...
    def test_process_single_pr_with_blocking_issues(self, pr_backlog_manager, sample_pr_data, mock_github_ops):
    ...
    def test_process_backlog_with_prs(self, pr_backlog_manager, mock_github_ops):
        ...
        # Mock PR discovery
        pr_backlog_manager.discover_prs_for_processing = Mock(return_value=[
            {'number': 123},
            {'number': 124},
            {'number': 125}
        ])
        ...
        ready_assessment = PRAssessment(
            pr_number=123, status=PRStatus.READY, criteria_met={}, 
            blocking_issues=[], resolution_actions=[], 
            last_updated=datetime.now(), processing_time=1.0
        )
        blocked_assessment = PRAssessment(
            pr_number=124, status=PRStatus.BLOCKED, criteria_met={}, 
            blocking_issues=['test issue'], resolution_actions=[], 
            last_updated=datetime.now(), processing_time=1.0
        )
        
        pr_backlog_manager.process_single_pr = Mock(side_effect=[
            ready_assessment, blocked_assessment, ready_assessment
        ])
