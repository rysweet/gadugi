...
try:
    from readiness_assessor import (
        ReadinessAssessor, ConflictAssessment, CIAssessment, ReviewAssessment,
        SyncAssessment, MetadataAssessment, ConflictComplexity, CIFailureType
    )
except ImportError:
...
class TestConflictAssessment:
    ...
    def test_assess_merge_conflicts_dirty(self, assessor):
        ...
        assert assessment.complexity in [ConflictComplexity.LOW, ConflictComplexity.MEDIUM]
    ...
    def test_estimate_resolution_time(self, assessor):
        ...
        assert assessor._estimate_resolution_time(ConflictComplexity.MEDIUM) == timedelta(minutes=45)
...
class TestCIAssessment:
    ...
    def test_assess_ci_status_all_passing(self, assessor, mock_github_ops, sample_pr_details):
        ...
        mock_github_ops.get_pr_status_checks.return_value = [
            {'state': 'success', 'context': 'ci/test', 'updated_at': '2024-01-01T12:00:00Z'},
            {'state': 'success', 'context': 'ci/build', 'updated_at': '2024-01-01T12:05:00Z'}
        ]
    ...
    def test_assess_ci_status_with_failures(self, assessor, mock_github_ops, sample_pr_details):
        ...
        mock_github_ops.get_pr_status_checks.return_value = [
            {'state': 'success', 'context': 'ci/test', 'updated_at': '2024-01-01T12:00:00Z'},
            {'state': 'failure', 'context': 'ci/build', 'description': 'Build failed due to timeout', 'updated_at': '2024-01-01T12:05:00Z'},
            {'state': 'error', 'context': 'ci/lint', 'description': 'Linting errors found', 'updated_at': '2024-01-01T12:03:00Z'}
        ]
    ...
    def test_classify_ci_failure(self, assessor):
        ...
        # Transient failure
        timeout_check = {'description': 'Build failed due to timeout', 'context': 'ci/build'}
        ...
        # Security failure
        security_check = {'description': 'Vulnerability detected', 'context': 'security/scan'}
        assert assessor._classify_ci_failure(security_check) == CIFailureType.SECURITY_FAILURE
        
        # Unknown failure
        unknown_check = {'description': 'Something went wrong', 'context': 'unknown/check'}
...
class TestReviewAssessment:
    ...
    def test_assess_review_status_complete(self, assessor, mock_github_ops, sample_pr_details):
        ...
        mock_github_ops.get_pr_reviews.return_value = [
            ...
            {'user': {'login': 'bot[bot]'}, 'state': 'APPROVED'}  # Should be filtered out
        ]
...
class TestComprehensiveAssessment:
    ...
    def test_get_comprehensive_assessment_ready(self, assessor, mock_github_ops, sample_pr_details):
        ...
        with patch.object(assessor, 'assess_merge_conflicts') as mock_conflicts, \
            ...
            mock_conflicts.return_value = ConflictAssessment(
                has_conflicts=False, affected_files=[], complexity=ConflictComplexity.NONE,
                resolution_estimate=timedelta(0), auto_resolvable=True
            )
            mock_ci.return_value = CIAssessment(
                all_passing=True, failing_checks=[], retriable_failures=[],
                blocking_failures=[], last_run=datetime.now()
            )
            mock_reviews.return_value = ReviewAssessment(
                has_approved_review=True, pending_requests=[], requested_changes=[],
                ai_review_complete=True, review_coverage_score=95.0
            )
            mock_sync.return_value = SyncAssessment(
                is_up_to_date=True, commits_behind=0, commits_ahead=2,
                last_sync=datetime.now(), sync_complexity="simple"
            )
            mock_metadata.return_value = MetadataAssessment(
                has_conventional_title=True, has_description=True,
                has_appropriate_labels=True, has_linked_issues=True,
    ...
    def test_get_comprehensive_assessment_blocked(self, assessor, mock_github_ops, sample_pr_details):
        ...
        with patch.object(assessor, 'assess_merge_conflicts') as mock_conflicts, \
            ...
            mock_conflicts.return_value = ConflictAssessment(
                has_conflicts=True, affected_files=['file1.py'], complexity=ConflictComplexity.MEDIUM,
                resolution_estimate=timedelta(minutes=30), auto_resolvable=False
            )
            mock_ci.return_value = CIAssessment(
                all_passing=False, failing_checks=[{'context': 'ci/test'}],
                retriable_failures=[], blocking_failures=['ci/test'], last_run=datetime.now()
            )
            mock_reviews.return_value = ReviewAssessment(
                has_approved_review=False, pending_requests=['reviewer1'],
                requested_changes=[], ai_review_complete=False, review_coverage_score=30.0
            )
            mock_sync.return_value = SyncAssessment(
                is_up_to_date=False, commits_behind=5, commits_ahead=2,
                last_sync=datetime.now(), sync_complexity="moderate"
            )
            mock_metadata.return_value = MetadataAssessment(
                has_conventional_title=False, has_description=True,
                has_appropriate_labels=False, has_linked_issues=False,
    ...
    def test_identify_blocking_factors(self, assessor):
        ...
        assessments = {
            'conflicts': ConflictAssessment(
                has_conflicts=True, affected_files=[], complexity=ConflictComplexity.HIGH,
                resolution_estimate=timedelta(0), auto_resolvable=False
            ),
            'ci': CIAssessment(
                all_passing=False, failing_checks=[{}, {}], retriable_failures=[],
                blocking_failures=[], last_run=None
            ),
            'reviews': ReviewAssessment(
                has_approved_review=False, pending_requests=[], requested_changes=[],
                ai_review_complete=True, review_coverage_score=0
            ),
            'sync': SyncAssessment(
                is_up_to_date=False, commits_behind=15, commits_ahead=0,
                last_sync=None, sync_complexity="complex"
            ),
            'metadata': MetadataAssessment(
                has_conventional_title=True, has_description=False,
                has_appropriate_labels=True, has_linked_issues=False,
    ...
    def test_generate_recommendations(self, assessor):
        ...
        assessments = {
            'conflicts': ConflictAssessment(
                has_conflicts=True, affected_files=[], complexity=ConflictComplexity.LOW,
                resolution_estimate=timedelta(0), auto_resolvable=True
            ),
            'ci': CIAssessment(
                all_passing=False, failing_checks=[], retriable_failures=['test'],
                blocking_failures=[], last_run=None
            ),
            'reviews': ReviewAssessment(
                has_approved_review=False, pending_requests=[], requested_changes=[],
                ai_review_complete=False, review_coverage_score=0
            ),
            'sync': SyncAssessment(
                is_up_to_date=False, commits_behind=3, commits_ahead=0,
                last_sync=None, sync_complexity="simple"
            ),
            'metadata': MetadataAssessment(
                has_conventional_title=False, has_description=False,
                has_appropriate_labels=False, has_linked_issues=False,
        ...
        assert any('auto-resolve' in rec.lower() for rec in recommendations)  # Auto-resolvable conflicts
        assert any('retry' in rec.lower() for rec in recommendations)  # Retriable CI failures
        assert any('human code review' in rec.lower() for rec in recommendations)  # Missing human review
