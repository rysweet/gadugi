...
import sys
import os
source_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', '.claude', 'agents', 'pr-backlog-manager')
sys.path.insert(0, source_path)

try:
    from delegation_coordinator import (
        DelegationCoordinator, DelegationTask, DelegationType, 
        DelegationPriority, DelegationStatus
    )
except ImportError:
...
class TestDelegationCreation:
    ...
    def test_classify_issue_type(self, coordinator):
        ...
        assert coordinator._classify_issue_type("PR has merge conflicts") == DelegationType.MERGE_CONFLICT_RESOLUTION
        
        # Test CI failure classification
        assert coordinator._classify_issue_type("CI checks are failing") == DelegationType.CI_FAILURE_FIX
        
        # Test branch update classification
        assert coordinator._classify_issue_type("Branch is behind main") == DelegationType.BRANCH_UPDATE
        
        # Test AI review classification
        assert coordinator._classify_issue_type("AI code review not completed") == DelegationType.AI_CODE_REVIEW
        
        # Test metadata classification
        assert coordinator._classify_issue_type("PR metadata incomplete") == DelegationType.METADATA_IMPROVEMENT
        
        # Test unknown issue (defaults to CI failure)
        assert coordinator._classify_issue_type("Some unknown issue") == DelegationType.CI_FAILURE_FIX
    
    def test_assess_issue_priority(self, coordinator):
        """Test issue priority assessment."""
        # Test critical priority
        pr_context = {'labels': []}
        assert coordinator._assess_issue_priority("Critical security issue", pr_context) == DelegationPriority.CRITICAL
        
        # Test high priority from labels
        pr_context = {'labels': ['priority-high']}
        assert coordinator._assess_issue_priority("Some issue", pr_context) == DelegationPriority.HIGH
        
        # Test medium priority from labels
        pr_context = {'labels': ['priority-medium']}
        assert coordinator._assess_issue_priority("Some issue", pr_context) == DelegationPriority.MEDIUM
        
        # Test merge conflict priority
        pr_context = {'labels': []}
        assert coordinator._assess_issue_priority("merge conflict issue", pr_context) == DelegationPriority.HIGH
        
        # Test CI failure priority
        assert coordinator._assess_issue_priority("CI failing", pr_context) == DelegationPriority.MEDIUM
        
        # Test default priority
        assert coordinator._assess_issue_priority("metadata issue", pr_context) == DelegationPriority.LOW
    
    def test_select_target_agent(self, coordinator):
        """Test target agent selection."""
        # Test WorkflowMaster capabilities
        assert coordinator._select_target_agent(DelegationType.MERGE_CONFLICT_RESOLUTION) == "workflow-master"
        ...
        assert coordinator._select_target_agent(DelegationType.METADATA_IMPROVEMENT) == "workflow-master"
        
        # Test code-reviewer capabilities
        assert coordinator._select_target_agent(DelegationType.AI_CODE_REVIEW) == "code-reviewer"
...
class TestDelegationExecution:
    ...
    def test_execute_delegation_workflow_master(self, coordinator):
        ...
        with patch.object(coordinator, '_delegate_to_workflow_master') as mock_delegate, \
    ...
    def test_execute_delegation_code_reviewer(self, coordinator):
        ...
        with patch.object(coordinator, '_delegate_to_code_reviewer') as mock_delegate, \
             patch.object(coordinator, '_add_delegation_comment') as mock_comment:
    ...
    def test_execute_delegation_failure_retry(self, coordinator):
        ...
        with patch.object(coordinator, '_delegate_to_workflow_master', side_effect=Exception("Test error")):
            coordinator._execute_delegation(task)
...
class TestDelegationManagement:
    ...
    def test_get_active_delegations(self, coordinator):
        ...
        task1 = DelegationTask(
            task_id="test-123", pr_number=456, task_type=DelegationType.MERGE_CONFLICT_RESOLUTION,
            priority=DelegationPriority.HIGH, agent_target="workflow-master", prompt_template="Test",
            context={}, created_at=datetime.now(), status=DelegationStatus.IN_PROGRESS
        )
        task2 = DelegationTask(
            task_id="test-456", pr_number=789, task_type=DelegationType.CI_FAILURE_FIX,
            priority=DelegationPriority.MEDIUM, agent_target="workflow-master", prompt_template="Test",
            context={}, created_at=datetime.now(), status=DelegationStatus.COMPLETED
        )
        task3 = DelegationTask(
            task_id="test-789", pr_number=456, task_type=DelegationType.AI_CODE_REVIEW,
            priority=DelegationPriority.LOW, agent_target="code-reviewer", prompt_template="Test",
            context={}, created_at=datetime.now(), status=DelegationStatus.PENDING
        )
    ...
    def test_cleanup_completed_delegations(self, coordinator):
        ...
        old_completed_task = DelegationTask(
            task_id="old-123", pr_number=456, task_type=DelegationType.MERGE_CONFLICT_RESOLUTION,
            priority=DelegationPriority.HIGH, agent_target="workflow-master", prompt_template="Test",
            context={}, created_at=datetime.now() - timedelta(days=2), 
            status=DelegationStatus.COMPLETED, 
            completion_time=datetime.now() - timedelta(days=2)
        )
        
        recent_completed_task = DelegationTask(
            task_id="recent-456", pr_number=789, task_type=DelegationType.CI_FAILURE_FIX,
            priority=DelegationPriority.MEDIUM, agent_target="workflow-master", prompt_template="Test",
        ...
        active_task = DelegationTask(
            task_id="active-789", pr_number=123, task_type=DelegationType.AI_CODE_REVIEW,
            priority=DelegationPriority.LOW, agent_target="code-reviewer", prompt_template="Test",
            context={}, created_at=datetime.now(), status=DelegationStatus.IN_PROGRESS
        )
    ...
    def test_get_delegation_metrics(self, coordinator):
        ...
        completed_task1 = DelegationTask(
            task_id="comp-1", pr_number=123, task_type=DelegationType.MERGE_CONFLICT_RESOLUTION,
            priority=DelegationPriority.HIGH, agent_target="workflow-master", prompt_template="Test",
            context={}, created_at=datetime.now() - timedelta(minutes=10), 
            status=DelegationStatus.COMPLETED,
            completion_time=datetime.now() - timedelta(minutes=5)
        )
        
        completed_task2 = DelegationTask(
            task_id="comp-2", pr_number=456, task_type=DelegationType.CI_FAILURE_FIX,
            priority=DelegationPriority.MEDIUM, agent_target="workflow-master", prompt_template="Test",
            context={}, created_at=datetime.now() - timedelta(minutes=15), 
            status=DelegationStatus.COMPLETED,
            completion_time=datetime.now() - timedelta(minutes=5)
        )
        
        failed_task = DelegationTask(
            task_id="fail-1", pr_number=789, task_type=DelegationType.AI_CODE_REVIEW,
            priority=DelegationPriority.LOW, agent_target="code-reviewer", prompt_template="Test",
            context={}, created_at=datetime.now() - timedelta(minutes=5), 
            status=DelegationStatus.FAILED
        )
        
        in_progress_task = DelegationTask(
            task_id="prog-1", pr_number=321, task_type=DelegationType.BRANCH_UPDATE,
            priority=DelegationPriority.HIGH, agent_target="workflow-master", prompt_template="Test",
            context={}, created_at=datetime.now() - timedelta(minutes=3), 
            status=DelegationStatus.IN_PROGRESS
        )
