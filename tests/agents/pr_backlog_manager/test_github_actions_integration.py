...
try:
    from github_actions_integration import (
        GitHubActionsIntegration, GitHubContext, SecurityConstraints,
        GitHubEventType, ProcessingMode
    )
except ImportError:
...
class TestGitHubContext:
    ...
    def test_extract_pr_number_from_ref(self):
        ...
        with patch.dict(os.environ, {'GITHUB_REF': 'refs/pull/789/merge'}), \
...
class TestGitHubActionsIntegration:
    ...
    def test_initialization_invalid_environment(self, mock_pr_backlog_manager):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(RuntimeError, match="GitHub Actions integration requires GITHUB_ACTIONS=true"):
    ...
    def test_initialization_missing_token(self, mock_pr_backlog_manager):
        ...
        with patch.dict(os.environ, env_vars):
            with pytest.raises(RuntimeError, match="GitHub Actions integration requires GITHUB_TOKEN"):
                GitHubActionsIntegration(mock_pr_backlog_manager)
    ...
    def test_initialization_auto_approve_invalid_event(self, mock_pr_backlog_manager, github_env_vars):
...
class TestProcessingExecution:
    ...
    def test_execute_processing_single_pr_success(self, integration, mock_pr_backlog_manager):
        ...
        integration.determine_processing_mode = Mock(return_value=(
            ProcessingMode.SINGLE_PR, {'pr_number': 123, 'reason': 'test'}
        ))
...
class TestArtifactCreation:
    ...
    def test_set_github_outputs_single_pr(self, integration):
        ...
        with patch.dict(os.environ, {'GITHUB_OUTPUT': '/tmp/outputs.txt'}), \
             patch('builtins.open', mock_open()) as mock_file:
...
class TestRateLimitHandling:
    ...
    def test_should_throttle_processing_no_throttling(self, integration):
        ...
        integration.check_rate_limits = Mock(return_value={
