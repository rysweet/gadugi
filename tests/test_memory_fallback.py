...
try:
    from ..claude.shared.memory_fallback import (
        ...
        MarkdownMemoryBackend, SQLiteMemoryBackend, InMemoryBackend,
        MemoryFallbackChain, create_simple_fallback_chain
    )
    from ..claude.shared.memory_integration import (
        EnhancedAgentMemoryInterface, create_enhanced_memory_interface,
...
class TestInMemoryBackend:
    ...
    async def test_memory_filtering(self):
        ...
        short_memories = await backend.get_agent_memories(
            "test_agent", 
        ...
        long_memories = await backend.get_agent_memories(
            "test_agent",
            long_term_only=True
        )
...
class TestMarkdownBackend:
    ...
    async def test_knowledge_graph_storage(self, temp_storage):
        ...
        # Link nodes
        await backend.link_knowledge_nodes(
            node1.id,
            node2.id, 
            "relates_to",
            strength=0.7
...
class TestMemoryFallbackChain:
    ...
    async def test_fallback_with_memory_operations(self, temp_storage):
        ...
        tagged_memories = await chain.search_memories_by_tags(
            agent_id, 
            ["knowledge"]
        )
...
class TestMemoryFallbackIntegration:
    ...
    async def test_concurrent_access(self, temp_storage):
        ...
        tasks = [
            store_memory("agent_1", f"Memory {i}") 
            for i in range(10)
        ]
