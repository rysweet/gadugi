...
def verify_setup():
        ...
        if 'hooks' in settings:
            ...
            if 'SubagentStop' in settings['hooks']:
                ...
                if subagent_config['command'].endswith('teamcoach-subagent-stop.py'):
                    print(f"   ✅ SubagentStop hook configured correctly")
                    print(f"      • Command: {subagent_config['command']}")
                    print(f"      • Timeout: {subagent_config['timeout']}s")
                    checks_passed += 1
                else:
                    print(f"   ❌ SubagentStop hook misconfigured")
        else:
            print(f"   ❌ 'hooks' section missing")
    else:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"   ❌ Claude command not found in PATH")
