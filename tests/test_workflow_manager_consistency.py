...
class MockGitHubPR:
    def __init__(self, number, title, author="test-user", has_reviews=False, created_minutes_ago=0):
        self.number = number
        self.title = title
        self.author = author
        self.has_reviews = has_reviews
        self.created_at = (datetime.now() - timedelta(minutes=created_minutes_ago)).isoformat()
...
class MockWorkflowState:
    def __init__(self, task_id="test-task-123", phase_8_complete=False, phase_9_complete=False, pr_number=None):
...
class TestPhase9Enforcement(unittest.TestCase):
    ...
    def test_phase_9_automatic_invocation_after_pr_creation(self, mock_subprocess):
        ...
        mock_subprocess.return_value.stdout = f"https://github.com/test/repo/pull/{self.mock_pr_number}"
    ...
    def test_phase_9_state_validation_before_completion(self, mock_subprocess):
        ...
        mock_subprocess.return_value.stdout = '{"reviews": [{"author": {"login": "code-reviewer"}}]}'
...
class TestExecutionCompletion(unittest.TestCase):
    ...
    def test_execution_commitment_requirements(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as temp_prompt:
...
""")
    ...
    def test_anti_termination_safeguards(self):
        ...
        for safeguard, should_be_disabled in safeguards.items():
            self.assertFalse(should_be_disabled, f"Safeguard {safeguard} must be disabled")
    ...
    def test_progress_verification_checkpoints(self):
        ...
        def verify_phase_artifacts(phase, **kwargs):
            ...
            if phase in artifacts:
                ...
                return required_artifact in kwargs and kwargs[required_artifact] is not None
...
class TestOrphanedPRDetection(unittest.TestCase):
    ...
    def setUp(self):
        self.script_path = "/Users/ryan/src/gadugi2/gadugi/.claude/utils/orphaned_pr_recovery.sh"
    ...
    def test_orphaned_pr_detection(self, mock_subprocess):
        ...
        orphaned_prs = [
            MockGitHubPR(101, "Feature Implementation", created_minutes_ago=10, has_reviews=False),
            MockGitHubPR(102, "Bug Fix Implementation", created_minutes_ago=6, has_reviews=False),
            MockGitHubPR(103, "Recent PR", created_minutes_ago=2, has_reviews=False),  # Too recent
            MockGitHubPR(104, "PR with Review", created_minutes_ago=10, has_reviews=True)  # Has review
        ]
    ...
    def test_orphaned_pr_recovery_script_exists(self):
        ...
        self.assertTrue(os.path.exists(self.script_path), "Orphaned PR recovery script should exist")
    ...
    def test_workflow_pr_identification(self, mock_subprocess):
        ...
        def is_workflow_pr(pr_title, pr_body=""):
            ...
            workflow_patterns = ["Implementation", "Feature.*Implementation", "Fix.*Implementation"]
...
class TestStateConsistencyValidation(unittest.TestCase):
    ...
    def test_phase_8_complete_but_no_review(self, mock_subprocess):
        ...
        state = MockWorkflowState(pr_number=123, phase_8_complete=True, phase_9_complete=False)
        with open(self.state_file, 'w') as f:
            f.write(state.to_state_file_content())
                ...
                if pr_match:
                    ...
                    if result.returncode == 0:
                        ...
                        if len(reviews_data.get('reviews', [])) == 0:
                            return True, f"CRITICAL: Phase 8 complete but no review for PR #{pr_number}"
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    @patch('time.sleep')
    @patch('os.system')
    def test_complete_workflow_with_phase_9_enforcement(self, mock_system, mock_sleep, mock_subprocess):
        ...
        # Execute phases 1-8
        for i in range(1, 9):
            phase_name = ["Setup", "Issue", "Branch", "Research", "Implementation", "Testing", "Documentation", "PR"][i-1]
