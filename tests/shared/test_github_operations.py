...
class TestGitHubOperations:
    ...
    def test_execute_gh_command_retry_logic(self, mock_sleep, mock_run):
        ...
        error2 = subprocess.CalledProcessError(1, ['gh', 'issue', 'list'], stderr='Connection error')
        success = Mock(returncode=0, stdout='{"success": true}', stderr='')
        
        mock_run.side_effect = [error1, error2, success]
        
        gh = GitHubOperations(retry_config={'max_retries': 3, 'initial_delay': 0.1, 'backoff_factor': 2})
        result = gh._execute_gh_command(['issue', 'list'])
    ...
    def test_execute_gh_command_max_retries_exceeded(self, mock_run):
        ...
        error = subprocess.CalledProcessError(1, ['gh', 'issue', 'list'], stderr='Persistent failure')
        mock_run.side_effect = [error, error]  # Two failures
        
        gh = GitHubOperations(retry_config={'max_retries': 2, 'initial_delay': 0.01, 'backoff_factor': 1})
        result = gh._execute_gh_command(['issue', 'list'])
    ...
    def test_create_issue_success(self, mock_run):
        ...
        mock_run.return_value.returncode = 0
        mock_run.return_value.stdout = '{"number": 42, "url": "https://github.com/test/repo/issues/42"}'
    ...
    def test_create_pr_success(self, mock_run):
        ...
        mock_run.return_value.returncode = 0
        mock_run.return_value.stdout = '{"number": 15, "url": "https://github.com/test/repo/pull/15"}'
