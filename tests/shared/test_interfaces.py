...
try:
    ...
    print("Warning: Could not import interfaces module. Tests will define what needs to be implemented.")
    ...
    def test_complete_workflow_interface_integration(self):
        ...
        class MockTaskTracker(TaskTrackerInterface):
            ...
            def create_task(self, content: str, priority: str = "medium", **kwargs) -> Dict[str, Any]:
                task = {"id": f"task-{len(self.tasks) + 1}", "content": content, "priority": priority}
        ...
        class MockGitHubOps(GitHubOperationsInterface):
            ...
            def create_pr(self, title: str, body: str, base: str, head: str, **kwargs) -> Dict[str, Any]:
                pr = {"number": len(self.prs) + 1, "title": title, "body": body, "base": base, "head": head}
                self.prs.append(pr)
                ...
                if component_type == "state_manager":
                    ...
                    class SimpleTaskTracker(TaskTrackerInterface):
                        ...
                        def create_task(self, content: str, priority: str = "medium", **kwargs) -> Dict[str, Any]:
                            ...
                            self.tasks.append({"id": task_id, "content": content, "status": "pending", "priority": priority})
                            return {"success": True, "task_id": task_id}
                        def update_task_status(self, task_id: str, status: str) -> Dict[str, Any]:
                            for task in self.tasks:
                                if task["id"] == task_id:
        ...
        class TestAgentFactory(AgentFactory):
            ...
            def create_agent(self, agent_type: str, config: AgentConfig) -> AgentInterface:
                if agent_type == "workflow_agent":
                    ...
                    # Create components
                    components = {
                        "state_manager": self.component_factory.create_component("state_manager", {}),
