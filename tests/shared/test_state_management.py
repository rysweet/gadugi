...
class TestWorkflowPhase:
    ...
    def test_workflow_phase_names(self):
        ...
        assert WorkflowPhase.get_phase_name(0) == 'Task Initialization & Resumption Check'
...
class TestCheckpointManager:
    ...
    def test_create_checkpoint(self, checkpoint_manager):
        ...
        assert checkpoint_file_compressed.exists() or checkpoint_file_uncompressed.exists()
    ...
    def test_restore_checkpoint(self, checkpoint_manager):
        ...
        checkpoint_id = checkpoint_manager.create_checkpoint(original_state, 'Backup point')
        
        # Restore checkpoint
        restored_state = checkpoint_manager.restore_checkpoint('restore-checkpoint', checkpoint_id)
    ...
    def test_checkpoint_compression(self, checkpoint_manager):
        ...
        checkpoint_file = checkpoint_manager.checkpoint_dir / 'compression-test' / f'{checkpoint_id}.json.gz'
        assert checkpoint_file.exists()
        
        # Verify we can still restore from compressed checkpoint
        restored_state = checkpoint_manager.restore_checkpoint('compression-test', checkpoint_id)
        assert restored_state is not None
...
class TestStateManagementIntegration:
    ...
    def test_error_recovery_integration(self, integration_setup):
        ...
        for phase in [1, 2, 3, 5]:
            ...
            state_manager.update_state(task_state)
            checkpoint_manager.create_checkpoint(task_state, f'Phase {phase} checkpoint')
        ...
        restored_state = checkpoint_manager.restore_checkpoint('error-recovery-test', phase_5_checkpoint)
