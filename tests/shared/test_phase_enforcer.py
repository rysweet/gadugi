...
from phase_enforcer import (
    PhaseEnforcer, EnforcementRule, EnforcementResult,
    enforce_phase_9, enforce_phase_10
)
...
class TestPhaseEnforcer:
    ...
    def test_enforce_code_review_github_cli_fallback(self, mock_subprocess):
        ...
        mock_subprocess.side_effect = [
            ...
            Mock(returncode=0, stdout="Review posted", stderr="")      # GitHub CLI succeeds
        ]
    ...
    def test_enforce_code_review_comment_fallback(self, mock_subprocess):
        ...
        mock_subprocess.side_effect = [
            Mock(returncode=1, stdout="", stderr="Claude failed"),     # Claude fails
            Mock(returncode=1, stdout="", stderr="Script failed"),     # Script fails
            Mock(returncode=1, stdout="", stderr="Review failed"),     # GitHub review fails
    ...
    def test_enforce_review_response_no_changes_needed(self, mock_subprocess):
        ...
        mock_subprocess.return_value = Mock(
            returncode=0, 
            stdout=json.dumps(mock_review_data), 
            stderr=""
        )
