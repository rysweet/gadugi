...
try:
    ...
    print("Warning: Could not import error_handling module. Tests will define what needs to be implemented.")
...
class TestRetryDecorator:
    ...
    def test_retry_exponential_backoff(self):
        ...
        with patch('time.sleep', side_effect=mock_sleep):
            @retry(max_attempts=3, initial_delay=0.1, 
                   strategy=RetryStrategy.EXPONENTIAL, backoff_factor=2.0)
            def always_fails():
    ...
    def test_retry_linear_backoff(self):
        ...
        with patch('time.sleep', side_effect=mock_sleep):
            @retry(max_attempts=3, initial_delay=0.1, 
                   strategy=RetryStrategy.LINEAR, backoff_factor=2.0)
            def always_fails():
                raise ValueError("Test")
