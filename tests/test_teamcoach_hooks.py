...
class TestTeamCoachSubagentStopHook(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(os.access(self.hook_script, os.X_OK), "Subagent hook script should be executable")
    ...
    def test_hook_processes_agent_data(self, mock_run):
        ...
        result = subprocess.run([
            'python', self.hook_script
        ], input=json.dumps(agent_data), text=True, capture_output=True)
...
class TestTeamCoachHookConfiguration(unittest.TestCase):
    ...
    def test_hooks_configuration_exists(self):
        ...
        self.assertIsInstance(subagent_hooks, list, "SubagentStop hooks should be a list")
        self.assertTrue(len(subagent_hooks) > 0, "Should have at least one SubagentStop hook")
