...
show_correct_usage() {
    ...
    print_colored "$BLUE" "   python .gadugi/.gadugi/src/orchestrator/main.py \\"
    print_colored "$BLUE" "     --task \"$task_description\" \\"
    print_colored "$BLUE" "     --auto-approve"
    echo
    print_colored "$GREEN" "📚 For more info: cat .gadugi/.gadugi/src/workflow-enforcement/workflow-reminder.md"
    echo
}
