...
class WorkflowReflectionCollector:
    ...
    def __init__(self, project_root: str = None):  # type: ignore[assignment]
    ...
    def _aggregate_session_data(
        ...
        aggregated = {
            ...
            "total_commits": sum(
                s.get("git_metrics", {}).get("recent_commits", 0) for s in session_data
            ),
            "total_tests": max(
            ...
            "issues_created": sum(
                s.get("issue_metrics", {}).get("recent_issues", 0) for s in session_data
            ),
            "prs_created": sum(
                s.get("issue_metrics", {}).get("recent_prs", 0) for s in session_data
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--generate-reflection",
        action="store_true",
        help="Generate reflection from recent data",
    )
    parser.add_argument(
