...
class WorkflowMonitoringState:
    ...
    stage_history: List[Tuple[WorkflowStage, datetime, datetime]] = field(
...
class WorkflowReliabilityManager:
    ...
    def start_workflow_monitoring(
        self, workflow_id: str, workflow_context: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Failed to start workflow monitoring for {workflow_id}: {e}"
            )
            return False
    ...
    def update_workflow_stage(
        ...
        try:
            if workflow_id not in self.monitoring_states:
                self.logger.error(
                    f"Workflow {workflow_id} not found in monitoring states"
                )
                return False
            ...
            stage_duration = (
                stage_end_time - monitoring_state.stage_start_time
            ).total_seconds()
            ...
            self.workflow_logger.info(
                ...
                extra={
                    ...
                    "total_duration": (
                        stage_end_time - monitoring_state.start_time
                    ).total_seconds(),
            ...
            if new_stage in critical_stages:
                ...
                if health_check.status in [HealthStatus.CRITICAL, HealthStatus.FAILED]:
                    self.workflow_logger.warning(
                        "Health check failed during stage transition",
                        extra={
                            "workflow_id": workflow_id,
    ...
    def perform_health_check(self, workflow_id: str) -> SystemHealthCheck:
        ...
        try:
            ...
            elif len(health_issues) == 1 and health_issues[0] not in [
                "github_connectivity",
                "claude_unavailable",
            ]:
    ...
    def handle_workflow_error(
        ...
        try:
            ...
            self.logger.error(
                f"Error handling failed for {workflow_id}: {recovery_error}"
            )
            return {
                "success": False,
    ...
    def check_workflow_timeouts(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            stage_duration = (
                current_time - monitoring_state.stage_start_time
            ).total_seconds()
    ...
    def create_workflow_persistence(
        self, workflow_id: str, workflow_state: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if success:
                self.workflow_logger.info(
                    ...
                    extra={
                        "workflow_id": workflow_id,
                        "stage": self.monitoring_states[workflow_id].current_stage.value
                        if workflow_id in self.monitoring_states
                        else "unknown",
            ...
            self.logger.error(
                f"Failed to create workflow persistence for {workflow_id}: {e}"
            )
            return False

    def restore_workflow_from_persistence(
        ...
        try:
            ...
            if monitoring_data:
                restored_monitoring = WorkflowMonitoringState(
                    ...
                    stage_start_time=datetime.fromisoformat(
                        monitoring_data["stage_start_time"]
                    ),
                    last_heartbeat=datetime.now(),
                    ...
                    stage_history=[
                        ...
                        for stage, start, end in monitoring_data.get(
                            "stage_history", []
                        )
                    ],
                )
            ...
            self.workflow_logger.info(
                ...
                extra={
                    "workflow_id": workflow_id,
                    "restored_stage": monitoring_data["current_stage"]
                    if monitoring_data
                    else "unknown",
                    "error_count": monitoring_data.get("error_count", 0)
                    if monitoring_data
            ...
            self.logger.error(
                f"Failed to restore workflow from persistence for {workflow_id}: {e}"
            )
            return None

    def stop_workflow_monitoring(
        self, workflow_id: str, completion_status: str = "completed"
    ) -> bool:
        """
        ...
        try:
            ...
            self.logger.error(
                f"Failed to stop workflow monitoring for {workflow_id}: {e}"
            )
            return False
    ...
    def get_workflow_diagnostics(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_health = (
                monitoring_state.health_checks[-5:]
                if monitoring_state.health_checks
                else []
            )
            ...
            return {
                ...
                "status": "active",
                "total_duration": (
                    current_time - monitoring_state.start_time
                ).total_seconds(),
                "current_stage": {
            ...
            self.logger.error(
                f"Failed to get workflow diagnostics for {workflow_id}: {e}"
            )
            return {"error": str(e)}
    ...
    def _start_monitoring_thread(self):
        ...
        self._monitoring_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="WorkflowMonitoring"
        )
        ...
        while self._monitoring_active and not self._shutdown_event.is_set():
            try:
                ...
                for workflow_id in list(self.monitoring_states.keys()):
                    ...
                    monitoring_state = self.monitoring_states[workflow_id]
                    time_since_last_health = (
                        datetime.now()
                        - (
                            monitoring_state.health_checks[-1].timestamp
                            if monitoring_state.health_checks
                            else monitoring_state.start_time
                        )
                    ).total_seconds()
    ...
    def _check_github_connectivity(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["gh", "api", "user"], capture_output=True, timeout=10
            )
    ...
    def _execute_timeout_recovery(
        self, workflow_id: str, stage: WorkflowStage, timeout_config: WorkflowTimeout
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for action in timeout_config.recovery_actions:
                if action == "restart_implementation":
                    ...
                    recovery_actions_taken.append(
                        f"health_check_{health_check.status.value}"
                    )
            ...
            return {
                ...
                "next_steps": [
                    ...
                    "Manual intervention if timeout persists",
                ],
            }
        ...
        try:
            if workflow_id in self.monitoring_states:
                ...
                checkpoint_state = TaskState(  # type: ignore
                    task_id=workflow_id,
                    prompt_file=self.active_workflows.get(workflow_id, {}).get(
                        "prompt_file", "unknown"
                    ),
                    status="in_progress",
                ...
                checkpoint_id = self.checkpoint_manager.create_checkpoint(
                    checkpoint_state, f"Checkpoint at stage {stage.value}"
                )
    ...
    def _create_error_checkpoint(
        self, workflow_id: str, error: Exception, stage: WorkflowStage
    ):
    ...
    def _generate_performance_report(
        self, workflow_id: str, monitoring_state: WorkflowMonitoringState
    ) -> Dict[str, Any]:
        """Generate comprehensive performance report for workflow"""
        try:
            total_duration = (
                datetime.now() - monitoring_state.start_time
            ).total_seconds()
            ...
            performance_score = (
                (total_baseline / total_duration) if total_duration > 0 else 0
            )
            ...
            return {
                ...
                "error_rate": monitoring_state.error_count
                / max(len(monitoring_state.stage_history), 1),
                "recovery_rate": monitoring_state.recovery_attempts
                / max(monitoring_state.error_count, 1)
                if monitoring_state.error_count > 0
                else 0,
            }
...
def monitor_workflow(
    ...
    return WorkflowReliabilityContext(
        workflow_id, workflow_context, reliability_manager
    )
