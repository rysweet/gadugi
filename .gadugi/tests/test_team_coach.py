...
class TeamCoach:
    ...
    def get_learning_summary(self):
        ...
        return {
            ...
            "success_rate": sum(
                1 for h in self.history if h.get("result", {}).get("success", False)
            )
            / len(self.history)
            if self.history
            else 0,
        ...
        if action == "analyze_session":
            ...
            metrics = SessionMetrics(
                **{
                    ...
                    if k in SessionMetrics.__dataclass_fields__
                }
            )
            ...
            avg_perf = (
                sum(s.performance_score for s in self.session_history) / sessions if sessions else 0
            )
            return {
                "success": True,
                "report": {
