...
from src.src.shared.task_tracking import TaskStatus  # type: ignore[import]

try:
    from src.src.shared.state_management import (  # type: ignore[import]
    ...
    print(f"Warning: Could not import shared.state_management as state_management module: {e}")
    ...
    class TaskState:
        ...
        def from_dict(cls, data: Dict[str, Any]) -> "TaskState":
            ...
            if "current_phase" in data and isinstance(data["current_phase"], (int, str)):
                try:
    ...
    class StateManager:
        ...
        def update_state(self, task_id: str, **updates) -> TaskState:
            ...
            if state is None:
                raise StateError(f"Task state {task_id} not found", operation="update_state")
        ...
        def validate_state_consistency(
            ...
            for state in self.list_task_states():
                ...
                if state.status == "completed" and not state.result:
                    errors.append(f"Task {state.task_id} is completed but has no result")
    ...
    class CheckpointManager:
        def __init__(
            self,
            config: Optional[Union[Dict[str, Any], "StateManager"]] = None,
        ):
...
class TestTaskState:
    ...
    def test_task_state_creation_complete(self):
        ...
        assert state.error_info is not None and state.error_info["last_error"] == "Network timeout"
    ...
    def test_task_state_update_phase(self):
        ...
        state = TaskState(task_id="test-task-005", prompt_file="test.md", status="in_progress")
    ...
    def test_task_state_set_error(self):
        ...
        state = TaskState(task_id="test-task-006", prompt_file="test.md", status="in_progress")
        ...
        assert state.error_info["type"] == "network_error"  # type: ignore[index]
        assert state.error_info["message"] == "Connection failed"  # type: ignore[index]
        assert state.error_info["details"]["phase"] == 2  # type: ignore[index]
        assert state.error_info["details"]["retry_count"] == 3  # type: ignore[index]
...
class TestWorkflowPhase:
    ...
    def test_workflow_phases(self):
        ...
        assert WorkflowPhase.ISSUE_CREATION.value == 4  # type: ignore[union-attr]
        assert WorkflowPhase.IMPLEMENTATION.value == 6  # type: ignore[union-attr]
        assert WorkflowPhase.REVIEW.value == 8  # type: ignore[union-attr]
    ...
    def test_workflow_phase_names(self):
        ...
        assert WorkflowPhase.get_phase_name(4) == "issue-creation"  # type: ignore[union-attr]
        assert WorkflowPhase.get_phase_name(6) == "implementation"  # type: ignore[union-attr]
        assert WorkflowPhase.get_phase_name(8) == "review"  # type: ignore[union-attr]
...
class TestStateManager:
    ...
    def test_state_manager_init_default(self):
        ...
        sm = StateManager()
        # The implementation uses .github/workflow-states as default and resolves to absolute path
        assert ".github/workflow-states" in str(sm.state_dir)
    ...
    def test_save_state_success(self, state_manager):
        ...
        # Verify file exists (in task-specific subdirectory)
    ...
    def test_update_state_success(self, state_manager):
        ...
        state = TaskState(
            task_id="test-update-001",
            prompt_file="update-test.md",
            status=TaskStatus.PENDING,  # type: ignore[arg-type]
        )
        state_manager.save_state(state)
    ...
    def test_list_states_by_status(self, state_manager):
        ...
        states = [  # type: ignore[misc]
            ...
            TaskState(task_id="progress-001", prompt_file="test3.md", status="in_progress"),
            TaskState(task_id="complete-001", prompt_file="test4.md", status=TaskStatus.COMPLETED),  # type: ignore[arg-type]
        ]
        ...
        # Filter by active
        active_states = state_manager.get_active_states()
        assert len(active_states) == 3  # 2 pending + 1 in_progress are active
    ...
    def test_backup_state(self, state_manager):
        ...
        state = TaskState(task_id="backup-test", prompt_file="backup.md", status="in_progress")
        state_manager.save_state(state)
    ...
    def test_restore_from_backup(self, state_manager):
        ...
        original_state_file = state_manager.state_dir / "restore-test" / "state.json"
...
class TestCheckpointManager:
    ...
    def test_list_checkpoints(self, checkpoint_manager):
        ...
        state = TaskState(task_id="list-checkpoints", prompt_file="test.md", status="in_progress")
        ...
        for i in range(3):
            ...
            checkpoint_id = checkpoint_manager.create_checkpoint(state, f"checkpoint-{i + 1}")
    ...
    def test_restore_checkpoint(self, checkpoint_manager):
        ...
        # Restore checkpoint
        restored_data = checkpoint_manager.restore_checkpoint("restore-checkpoint", checkpoint_id)
    ...
    def test_cleanup_old_checkpoints(self, checkpoint_manager):
        ...
        for i in range(7):
            ...
            checkpoint_manager.create_checkpoint(state, f"Checkpoint {i + 1} for cleanup")
    ...
    def test_checkpoint_compression(self, checkpoint_manager):
        ...
        checkpoint_id = checkpoint_manager.create_checkpoint(state, "Compression test checkpoint")
        ...
        restored_data = checkpoint_manager.restore_checkpoint("compression-test", checkpoint_id)
        assert restored_data is not None  # type: ignore[comparison-overlap]
