...
class TestEnvironmentVariableConfiguration(unittest.TestCase):
    ...
    def test_no_hardcoded_secrets_in_code(self):
        ...
        self.assertNotIn(
            config.password, forbidden_passwords, f"Found hardcoded password: {config.password}"
        )
    ...
    def test_docker_compose_uses_env_vars(self):
        ...
        if docker_compose_path.exists():
            ...
            self.assertIn(
                "${NEO4J_AUTH", content, "docker-compose.yml should use NEO4J_AUTH env var"
            )

            # Verify no hardcoded passwords
            self.assertNotIn("gadugi123", content, "Found hardcoded password in docker-compose.yml")
    ...
    def test_gitignore_includes_env_file(self):
        ...
        if gitignore_path.exists():
            ...
            self.assertIn(".env", content, ".env file must be in .gitignore for security")
