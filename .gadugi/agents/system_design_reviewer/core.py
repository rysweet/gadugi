...
class SystemDesignReviewer:
    ...
    def review_pr(
        self, pr_number: str, force_adr: bool = False, update_architecture: bool = True
    ) -> ReviewResult:
    ...
    def _analyze_pr_changes(
        self, pr_number: str, pr_info: Dict[str, Any]
        ...
        if len(analyzable_files) > self.max_pr_size:
            print(
                f"Warning: PR has {len(analyzable_files)} files, analyzing first {self.max_pr_size}"
            )
    ...
    def _generate_review_comments(
        self, changes: List[ArchitecturalChange], overall_impact: ImpactLevel
    ) -> List[str]:
        ...
        for change_type, type_changes in changes_by_type.items():
            ...
            for change in type_changes:
                ...
                comments.append(
                    f"- **{element.element_type.value}** `{element.name}` at {element.location}"
                )
                ...
                if change.impact_level in [ImpactLevel.HIGH, ImpactLevel.CRITICAL]:
                    comments.append(
                        f"  - ⚠️ {change.impact_level.value.title()} impact - review required"
                    )
