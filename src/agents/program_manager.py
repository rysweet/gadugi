...
class ProgramManager:
    ...
    def __init__(self, agent_id: str = "pm-001"):
        ...
        self.lifecycle_labels = [stage.value for stage in IssueStage if stage != IssueStage.UNLABELED]
    ...
    def get_issues_by_label(self, label: Optional[str] = None) -> List[Issue]:
        ...
        args = ["issue", "list", "--json", "number,title,body,labels,state,author,createdAt,updatedAt"]
        if label:
    ...
    def get_current_lifecycle_stage(self, issue: Issue) -> Optional[IssueStage]:
        ...
        lifecycle_labels_in_issue = [label for label in issue.labels if label in self.lifecycle_labels]
        ...
        if len(lifecycle_labels_in_issue) > 1:
            print(f"Warning: Issue #{issue.number} has multiple lifecycle labels: {lifecycle_labels_in_issue}")
    ...
    def _looks_like_bug(self, issue: Issue) -> bool:
        ...
        bug_keywords = ['bug', 'error', 'crash', 'broken', 'fix', 'issue', 'problem', 'fail']
    ...
    def _is_well_structured(self, issue: Issue) -> bool:
        ...
        structure_indicators = ['##', '###', '- ', '* ', '1.', 'requirements:', 'description:', 'acceptance criteria:']
        body_lower = issue.body.lower()
        
        structure_count = sum(1 for indicator in structure_indicators if indicator in body_lower)
        return structure_count >= 2
    
    def update_issue_label(self, issue_number: int, add_label: str, remove_label: Optional[str] = None) -> bool:
        """Update issue labels"""
    ...
    def triage_unlabeled_issues(self) -> Dict[str, int]:
        ...
        for issue in unlabeled:
            ...
            if any(label in self.lifecycle_labels for label in issue.labels):
                print(f"  Skipping - already has lifecycle label")
                continue
...
"""
    ...
    def _extract_problem(self, body: str) -> str:
        ...
        return '\n'.join(problem_lines) if problem_lines else "TODO: Define the problem this issue addresses"
    ...
    def enforce_single_lifecycle_label(self) -> Dict[str, int]:
        ...
        # Get all issues
        success, output = self.run_gh_command(["issue", "list", "--json", "number,labels", "--limit", "1000"])
        if not success:
            print(f"Failed to get issues: {output}")
            return stats
        ...
        try:
            ...
            for issue_data in all_issues:
                ...
                lifecycle_labels_in_issue = [label for label in labels if label in self.lifecycle_labels]
                
                if len(lifecycle_labels_in_issue) > 1:
                    print(f"\nIssue #{issue_data['number']} has multiple lifecycle labels: {lifecycle_labels_in_issue}")
                    
                    # Keep the most advanced stage
                    stages_order = [IssueStage.IDEA, IssueStage.DRAFT, IssueStage.REQUIREMENTS_REVIEW,
                                  IssueStage.DESIGN_READY, IssueStage.DESIGN_REVIEW, IssueStage.READY]
                    ...
                    if kept_label:
                        ...
                        for label in lifecycle_labels_in_issue:
                            if label != kept_label:
                                self.run_gh_command(["issue", "edit", str(issue_data['number']), 
                                                   "--remove-label", label])
                        stats["fixed"] += 1
    ...
    def get_milestone_info(self) -> Dict[str, any]:
        ...
        success, output = self.run_gh_command(["api", "/repos/{owner}/{repo}/milestones", "--jq", "."])
        if not success:
    ...
    def analyze_ready_issues(self) -> List[Dict[str, any]]:
        ...
        for issue in ready_issues:
            ...
            has_blockers = any(word in issue.body.lower() for word in ['blocked', 'depends on', 'waiting for'])
    ...
    def update_project_priorities(self) -> bool:
        ...
        if blocked_ready:
            priorities.append(f"{priority_number}. **Unblock ready issues**: {len(blocked_ready)} issues blocked")
            priority_number += 1
        ...
        if unblocked_ready:
            ...
            priorities.append(f"{priority_number}. **Implement ready issues**: {ready_list} (and {len(unblocked_ready)-3} more)")
            priority_number += 1
            
        # Priority 3: Move design-review to ready
        if stage_counts.get('design-review', 0) > 0:
            priorities.append(f"{priority_number}. **Complete design reviews**: {stage_counts['design-review']} issues awaiting review")
            priority_number += 1
            
        # Priority 4: Process requirements-review
        if stage_counts.get('requirements-review', 0) > 0:
            priorities.append(f"{priority_number}. **Complete requirements reviews**: {stage_counts['requirements-review']} issues need review")
            priority_number += 1
            
        # Priority 5: Structure ideas
        if stage_counts.get('idea', 0) > 5:
            priorities.append(f"{priority_number}. **Structure idea backlog**: {stage_counts['idea']} ideas need structuring")
            priority_number += 1
...
"""
        ...
        # Add pipeline status
        for stage in [IssueStage.IDEA, IssueStage.DRAFT, IssueStage.REQUIREMENTS_REVIEW, 
                     IssueStage.DESIGN_READY, IssueStage.DESIGN_REVIEW, IssueStage.READY]:
        ...
        if stage_counts.get('idea', 0) > stage_counts.get('ready', 0) * 2:
            priority_content += "- ⚠️ **Idea backlog growing**: Consider dedicated grooming session\n"
...
"""
        ...
        if new_agents:
            ...
            if agents_section_start != -1:
                ...
                for agent in new_agents:
                    ...
                    updated_readme = (updated_readme[:insert_pos] + 
                                    agent_line + 
        ...
        try:
            ...
            self.memory.record_agent_memory(
                "readme_update",
                f"Updated README with {', '.join(update_summary)}"
            )
