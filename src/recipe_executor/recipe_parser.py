...
from .recipe_model import (
    ...
    Requirement, RequirementPriority, ComponentDesign, Interface,
    ComponentType
)
...
class RecipeParser:
    ...
    def _extract_purpose(self, content: str) -> str:
        ...
        purpose_match = re.search(r"##?\s*(?:Core\s+)?Purpose\s*\n(.*?)(?:\n##|\Z)", content, re.DOTALL)
        if purpose_match:
    ...
    def _extract_component_designs(self, content: str) -> list[ComponentDesign]:
        ...
        component_pattern = re.compile(r"###?\s*(?:\d+\.\s+)?(\w+(?:\s+\w+)*?)\s*\(`([^`]+)`\)", re.MULTILINE)
    ...
    def validate_recipe(self, recipe: Recipe) -> list[str]:
        ...
        if not recipe.get_outputs():
            issues.append("No outputs defined in components.json")
