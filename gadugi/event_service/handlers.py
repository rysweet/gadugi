...
from .config import EventHandlerConfig, EventFilter as EventFilterConfig, GitHubFilter as GitHubFilterConfig, AgentInvocation
...
class GitHubFilter:
    ...
    def matches(self, github_event: GitHubEvent) -> bool:
        ...
        if self.refs:
            if not any(self._matches_ref(pattern, github_event.ref) for pattern in self.refs):
                return False
...
class EventFilter:
    ...
    def matches(self, event: Event) -> bool:
        ...
        if self.event_types:
            if not any(self._matches_pattern(pattern, event.event_type) for pattern in self.event_types):
                return False
    ...
    def find_matching_handlers(self, event: Event) -> List[EventHandler]:
        ...
        for handler in self.handlers:
            if handler.matches(event):
                ...
                logger.debug(f"Event {event.event_type} matched handler: {handler.name}")
