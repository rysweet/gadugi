...
class AgentInvoker:
    ...
    async def invoke_agent(self, invocation: AgentInvocation, event: Event) -> Dict[str, Any]:
    ...
    async def _invoke_claude_cli(self, invocation: AgentInvocation, event: Event) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"Agent {invocation.agent_name} failed with return code {process.returncode}")
                logger.error(f"stderr: {result['stderr']}")
            
            return result
            
        except Exception as e:
            logger.error(f"Error invoking Claude CLI agent {invocation.agent_name}: {e}")
            return {
                "success": False,
                "error": str(e),
                "method": "claude_cli",
    ...
    async def _invoke_direct(self, invocation: AgentInvocation, event: Event) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Direct invocation not yet implemented for {invocation.agent_name}, using subprocess")
            return await self._invoke_subprocess(invocation, event)
    ...
    async def _invoke_subprocess(self, invocation: AgentInvocation, event: Event) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if success:
                    logger.info(f"Subprocess agent {invocation.agent_name} completed successfully")
                else:
                    logger.error(f"Subprocess agent {invocation.agent_name} failed with return code {process.returncode}")
            ...
            logger.error(f"Error invoking subprocess agent {invocation.agent_name}: {e}")
            return {
                "success": False,
                "error": str(e),
                "method": "subprocess",
    ...
    def _generate_prompt(self, invocation: AgentInvocation, event: Event) -> str:
        ...
        if not template:
            # Default template
            template = f"Process event: {event.event_type}\n\nEvent data: {event.to_json()}"
    ...
    def validate_invocation(self, invocation: AgentInvocation) -> bool:
        ...
        if invocation.method == "claude_cli":
            if not self._is_executable(self.claude_cli_path) and self.claude_cli_path != "claude":
