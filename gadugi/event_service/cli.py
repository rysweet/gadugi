...
from .config import (
    ServiceConfig, load_config, save_config, create_default_config,
    get_default_config_path, get_default_socket_path
)
...
Examples:
  ...
  gadugi logs --tail                 # Show service logs
        ...
        # Install command
        install_parser = subparsers.add_parser("install", help="Install and configure Gadugi service")
        install_parser.add_argument("--github-token", help="GitHub token for API access")
        install_parser.add_argument("--webhook-secret", help="GitHub webhook secret")
        install_parser.add_argument("--port", type=int, default=8080, help="Webhook server port")
        install_parser.add_argument("--no-webhook", action="store_true", help="Skip webhook setup")
        ...
        setup_parser = webhook_subparsers.add_parser("setup", help="Setup GitHub webhook")
        setup_parser.add_argument("--repo", help="Repository (owner/name)")
        setup_parser.add_argument("--url", help="Webhook URL (auto-detected if not provided)")
        ...
        list_handlers_parser = handler_subparsers.add_parser("list", help="List event handlers")
        
        enable_parser = handler_subparsers.add_parser("enable", help="Enable handler")
        enable_parser.add_argument("name", help="Handler name")
        
        disable_parser = handler_subparsers.add_parser("disable", help="Disable handler")
    ...
    async def install(self, args: argparse.Namespace) -> int:
        ...
        print(f"Start the service with: gadugi start")
        return 0
    ...
    async def config(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if args.show:
                ...
                new_interval = input(f"New poll interval ({config.poll_interval_seconds}): ").strip()
        ...
        try:
            ...
            if args.handler_action == "list":
                ...
                for handler in config.handlers:
                    ...
                    print(f"  {handler.name}: {handler.invocation['agent_name']} ({status})")
                return 0
    ...
    async def _setup_webhook_interactive(self, config: ServiceConfig):
        ...
        async with GitHubClient(config.github_token) as client:
            ...
            if repo_info:
                ...
                if confirm == 'y':
                    await client.create_webhook(owner, repo, webhook_url, config.webhook_secret)
                    print("Webhook created successfully!")
    
    async def _webhook_setup(self, client: GitHubClient, config: ServiceConfig, args: argparse.Namespace) -> int:
        ...
        result = await client.create_webhook(owner, repo, webhook_url, config.webhook_secret)
        print(f"Webhook created: ID {result.get('id')}")
        return 0
    
    async def _webhook_list(self, client: GitHubClient, args: argparse.Namespace) -> int:
    ...
    async def _webhook_test(self, client: GitHubClient, args: argparse.Namespace) -> int:
    ...
    async def _webhook_delete(self, client: GitHubClient, args: argparse.Namespace) -> int:
