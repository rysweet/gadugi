...
class GadugiEventService:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        logger.info(f"Gadugi Event Service initialized with {len(self.handlers)} handlers")
    ...
    async def _start_webhook_server(self):
        ...
        logger.info(f"Webhook server listening on {self.config.bind_address}:{self.config.bind_port}")
    ...
    async def _start_github_polling(self):
        ...
        logger.info(f"Starting GitHub polling every {self.config.poll_interval_seconds} seconds")
    ...
    async def _handle_github_webhook(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if hasattr(self, 'audit_logger'):
                self.audit_logger.info(f"Received GitHub webhook: {event_type} from {event.payload.github_event.repository}")
    ...
    async def _verify_webhook_signature(self, request: web.Request) -> bool:
    ...
    def _create_github_event(self, event_type: str, webhook_data: Dict[str, Any]) -> Event:
    ...
    async def _process_event(self, event: Event):
        ...
        logger.info(f"Processing event {event.event_type} with {len(matching_handlers)} handlers")
    ...
    async def _execute_handler(self, handler: EventHandler, event: Event):
        ...
        try:
            logger.info(f"Executing handler: {handler.name} for event: {event.event_type}")
            
            if hasattr(self, 'audit_logger'):
                self.audit_logger.info(f"Executing handler: {handler.name} -> {handler.invocation.agent_name}")
            ...
            logger.error(f"Handler {handler.name} timed out after {handler.timeout_seconds} seconds")
        except Exception as e:
