...
class LLMProviderBase(ABC):
    ...
    @abstractmethod
    async def generate_streaming_completion(self, request: LLMRequest) -> AsyncIterator[str]:
...
class ResponseCache:
    ...
    def put(self, request: LLMRequest, response: LLMResponse, ttl: Optional[int] = None):
        """Cache response."""
        if request.stream:  # Don't cache streaming requests
            return
...
class LoadBalancer:
    ...
    def select_provider(self, request: LLMRequest) -> Optional[LLMProviderBase]:
        ...
        available_providers = [
            ...
            if p.can_handle_request(request)
        ]
        ...
        with self.lock:
            if self.strategy == LoadBalanceStrategy.ROUND_ROBIN:
                ...
                return min(available_providers, key=lambda p: p.stats.average_response_time or float('inf'))
...
class LLMProxyService:
    ...
    async def _initialize_default_providers(self):
        ...
        mock_config = ModelConfig(
            ...
            capabilities=[ModelCapability.CHAT_COMPLETION, ModelCapability.TEXT_GENERATION],
            max_tokens=2048,
            cost_per_token=0.0,
            rate_limit_rpm=1000,
                ...
                try:
                    ...
                    if provider.can_handle_request(test_request):
                        ...
                        if isinstance(provider, MockProvider):
                            test_response = await provider.generate_completion(test_request)
...
def create_chat_request(messages: List[Dict[str, str]], 
                       model: str = "gpt-3.5-turbo",
                       temperature: float = 0.7,
                       max_tokens: int = 1000) -> LLMRequest:
...
def create_completion_request(prompt: str,
                            model: str = "gpt-3.5-turbo",
                            temperature: float = 0.7,
                            max_tokens: int = 1000) -> LLMRequest:
...
def create_function_call_request(messages: List[Dict[str, str]],
