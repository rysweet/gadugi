        ...
        try:
            with self.lock:
                ...
                if self.compression_enabled and len(content_json) > 1024:
                    content_json = base64.b64encode(gzip.compress(content_json.encode())).decode()
                    compressed = True
                ...
                with sqlite3.connect(str(self.db_path)) as conn:
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        memory.id, memory.type.value, content_json, context_json,
                        metadata_json, memory.created_at.isoformat(),
                        memory.accessed_at.isoformat(), memory.access_count,
                        memory.importance_score, memory.decay_rate, tags_json, compressed
                    ))
                ...
                return OperationResult(
                    ...
                    metadata={"compressed": compressed, "cache_size": len(self.memory_cache)},
            ...
            with self.lock:
                ...
                if memory_id in self.memory_cache:
                    ...
                    return OperationResult(
                        ...
                        data=memory,
                        metadata={"source": "cache", "access_count": memory.access_count},
                        execution_time=execution_time
                    )
                ...
                with sqlite3.connect(str(self.db_path)) as conn:
                    ...
                    conn.execute("""
                        ...
                        WHERE id = ?
                    """, (memory.access_count, memory.accessed_at.isoformat(), memory_id))
                ...
                return OperationResult(
                    ...
                    operation="retrieve_memory",
                    data=memory,
                    metadata={"source": "database", "access_count": memory.access_count},
                    execution_time=execution_time
                )
...
class ContextManager:
    ...
    def __init__(self, storage_path: str, redis_url: Optional[str] = None):
        ...
        if REDIS_AVAILABLE and redis_url:
            try:
                ...
                self.logger.warning(f"Redis connection failed, using local storage: {e}")
                self.redis_client = None
    ...
    def save_context(self, context: ContextSnapshot, persistence_level: PersistenceLevel = PersistenceLevel.SESSION) -> OperationResult:
        ...
        try:
            with self.lock:
                ...
                if len(context_json) > 1024:
                    context_json = base64.b64encode(gzip.compress(context_json.encode())).decode()
                    compressed = True
                
                if persistence_level in [PersistenceLevel.TEMPORARY, PersistenceLevel.SESSION]:
                    ...
                    self.redis_client.set(redis_key, json.dumps(redis_data), ex=expire_seconds)
                
                if persistence_level in [PersistenceLevel.PERMANENT, PersistenceLevel.ARCHIVAL]:
                    ...
                    with sqlite3.connect(str(self.db_path)) as conn:
                        conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            context.id, context.type.value, context_json, metadata_json,
                            context.created_at.isoformat(),
                            context.expires_at.isoformat() if context.expires_at else None,
                            compressed, context.checksum
                        ))
                        conn.commit()
    ...
    def list_contexts(self, context_type: Optional[ContextType] = None) -> OperationResult:
        ...
        try:
            with self.lock:
                ...
                # Get from Redis
                pattern = "context:*"
                redis_keys = self.redis_client.keys(pattern) if hasattr(self.redis_client, 'keys') else []
...
class MCPService:
    ...
    async def save_context(self, context: ContextSnapshot, persistence_level: PersistenceLevel = PersistenceLevel.SESSION) -> OperationResult:
        """Save a context snapshot."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            self.executor,
    ...
    async def list_contexts(self, context_type: Optional[ContextType] = None) -> OperationResult:
        """List available contexts."""
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(
            self.executor,
    ...
    async def store_episodic_memory(self, event_description: str, context: Dict[str, Any], importance: float = 1.0) -> OperationResult:
        """Store an episodic memory (event-based)."""
        memory = MemoryEntry(
            id=str(uuid.uuid4()),
            type=MemoryType.EPISODIC,
            content={"event": event_description, "timestamp": datetime.now().isoformat()},
            context=context,
    ...
    async def store_semantic_memory(self, knowledge: Dict[str, Any], category: str = "general", importance: float = 1.0) -> OperationResult:
    ...
    async def save_session_context(self, session_id: str, context_data: Dict[str, Any], expires_in_hours: int = 24) -> OperationResult:
    ...
    async def save_workflow_context(self, workflow_id: str, context_data: Dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            service_stats = {
                ...
                "service_performance": {
                    ...
                    "average_operation_time": self.total_operation_time / self.operation_count if self.operation_count > 0 else 0,
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            context_result = await self.save_context(test_context, PersistenceLevel.TEMPORARY)
            ...
            while self.running:
                ...
                try:
                    ...
                    if result.success and result.data.get("deleted_count", 0) > 0:
                        self.logger.info(f"Cleaned up {result.data['deleted_count']} expired contexts")
...
def create_episodic_memory(event_description: str, context: Dict[str, Any], importance: float = 1.0, tags: List[str] = None) -> MemoryEntry:
...
def create_semantic_memory(knowledge: Dict[str, Any], category: str = "general", importance: float = 1.0) -> MemoryEntry:
...
def create_session_context(session_id: str, context_data: Dict[str, Any], expires_in_hours: int = 24) -> ContextSnapshot:
...
def create_workflow_context(workflow_id: str, context_data: Dict[str, Any]) -> ContextSnapshot:
