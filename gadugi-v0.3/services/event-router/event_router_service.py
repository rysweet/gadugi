...
class EventQueue:
    ...
    async def put(self, event: Event):
        ...
        if self._total_size >= self.maxsize:
            ...
            try:
                await asyncio.wait_for(self._queues[EventPriority.LOW].get(), timeout=0.1)
                self._total_size -= 1
    ...
    async def get(self) -> Event:
        ...
        for priority in [EventPriority.CRITICAL, EventPriority.HIGH, EventPriority.NORMAL, EventPriority.LOW]:
            try:
                event = await asyncio.wait_for(self._queues[priority].get(), timeout=0.1)
                with self._lock:
                    self._total_size -= 1
                return event
            except asyncio.TimeoutError:
...
class EventRouterService:
    ...
    async def stop(self):
        ...
        tasks = [task for task in [self.server_task, self.processor_task, self.cleanup_task] if task]
        if tasks:
    ...
    async def _handle_subscription(self, client_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.info(f"Created subscription {subscription.id} for client {client_id}")
        ...
        try:
            ...
            if subscription_id in self.subscriptions:
                ...
                if subscription.subscriber_id == client_id:
                    ...
                    self.logger.info(f"Removed subscription {subscription_id} for client {client_id}")
    ...
    async def _route_event(self, event: Event):
        ...
        if failed_count > 0:
            self.logger.warning(f"Failed to deliver event {event.id} to {failed_count} subscribers")
    ...
    def _event_matches_filter(self, event: Event, event_filter: Optional[EventFilter]) -> bool:
        ...
        try:
            while self.running:
                ...
                for sub_id, subscription in self.subscriptions.items():
                    ...
                    if (subscription.endpoint and 
                        subscription.endpoint not in self.active_connections):
    ...
    def subscribe(self, 
                  ...
                  callback: Optional[Callable[[Event], None]] = None) -> str:
...
class EventRouterClient:
    ...
    async def subscribe(self, 
                       event_filter: Optional[EventFilter] = None,
                       subscription_type: SubscriptionType = SubscriptionType.FILTERED) -> Optional[str]:
        """Subscribe to events."""
        if not self.connected:
            return None
