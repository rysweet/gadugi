...
class TestGadugiEngine:
    ...
    def test_core_services_configuration(self):
        ...
        expected_services = ["event-router", "neo4j-graph", "mcp-service", "llm-proxy", "gadugi-cli"]
    ...
    def test_available_agents_configuration(self):
        ...
        expected_agents = [
            "orchestrator", "architect", "task-decomposer", "workflow-manager",
            "code-writer", "code-reviewer", "memory-manager", "team-coach",
            "prompt-writer", "worktree-manager"
        ]
    ...
    def test_execute_operation_invalid_command(self):
        ...
        assert len(result.errors) > 0
        assert "invalid_command" in result.errors[0].lower() or "unsupported" in result.errors[0].lower()
    ...
    def test_handle_install_system(self):
        ...
        with patch.object(self.gadugi, '_install_dependencies') as mock_deps, \
    ...
    def test_create_backup_failure(self):
        ...
        with patch('gadugi_engine.tarfile.open', side_effect=Exception("Backup failed")):
            backup_file = self.gadugi._create_backup("full", True, True)
    ...
    def test_handle_optimize_operation(self):
        ...
        with patch.object(self.gadugi, '_optimize_system') as mock_optimize:
            mock_optimize.return_value = {"optimizations_applied": ["test optimization"]}
    ...
    def test_log_system_event(self):
        ...
        self.gadugi._log_system_event("test_event", "test_component", "Test message", details)
        
        # Verify database entry
        with sqlite3.connect(self.gadugi.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM system_events WHERE event_type = ?", ("test_event",))
            result = cursor.fetchone()
    ...
    def test_shutdown(self):
        ...
        self.gadugi.services["test-service"] = {"process": Mock(), "status": ServiceStatus.RUNNING}
        self.gadugi.agents["test-agent"] = {"process": Mock(), "status": "active"}
        self.gadugi.is_monitoring = True
        
        with patch.object(self.gadugi, '_stop_monitoring') as mock_stop_monitor, \
    ...
    def test_perform_health_check_healthy(self):
        ...
        with patch.object(self.gadugi, '_is_service_running', return_value=True), \
    ...
    def test_perform_health_check_critical(self):
        ...
        with patch.object(self.gadugi, '_is_service_running', return_value=False), \
    ...
    def test_restore_backup_success(self):
        ...
        with patch.object(backup_path, 'exists', return_value=True), \
             patch.object(self.gadugi, '_get_backup_checksum', return_value="test_checksum"), \
             patch.object(self.gadugi, '_calculate_file_checksum', return_value="test_checksum"), \
             patch('gadugi_engine.tarfile.open') as mock_tarfile:
            
            mock_tar = Mock()
            mock_tarfile.return_value.__enter__.return_value = mock_tar
            
            with patch.object(self.gadugi, '_stop_all_services'), \
    ...
    def test_restore_backup_checksum_mismatch(self):
        ...
        with patch.object(backup_path, 'exists', return_value=True), \
             patch.object(self.gadugi, '_get_backup_checksum', return_value="stored_checksum"), \
             patch.object(self.gadugi, '_calculate_file_checksum', return_value="different_checksum"):
    ...
    def test_error_handling_in_execute_operation(self):
        ...
        with patch.object(self.gadugi, '_handle_status', side_effect=Exception("Test error")):
            result = self.gadugi.execute_operation(request)
