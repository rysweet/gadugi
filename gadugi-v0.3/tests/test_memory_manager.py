...
from memory_manager_engine import (
    MemoryManager, GitHubSync, MemoryManagerEngine,
    MemoryItem, MemorySection, GitHubIssue, MemoryManagerRequest,
    run_memory_manager
)
...
"""
    ...
    def test_determine_item_type(self):
        ...
        assert self.memory_manager._determine_item_type("- Objective: Improve performance") == 'goal'
...
"""
    ...
    def test_handle_sync(self, mock_github_sync):
        ...
        mock_sync_instance.create_issue_for_item.return_value = GitHubIssue(
            number=123, title='Test', body='Test', state='open',
            labels=['memory-sync'], url='https://github.com/test/repo/issues/123'
        )
