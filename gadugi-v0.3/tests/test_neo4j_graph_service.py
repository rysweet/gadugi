...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'services', 'neo4j-graph'))
...
class TestNeo4jGraphService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_update_node_success(self, mock_graph_db):
        ...
        updated_data = {"id": "test-node", "name": "Updated Test Node", "status": "modified"}
    ...
    async def test_search_content_success(self, mock_graph_db):
        ...
        # Mock search results
        search_results = [
            {"node": {"id": "doc-001", "title": "Test Document", "content": "test content"}}
        ]
    ...
    async def test_execute_cypher_success(self, mock_graph_db):
        ...
        # Mock cypher results
        cypher_results = [
            {"node_name": "Test Node", "relationship_count": 5}
        ]
