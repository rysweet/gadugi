...
class MemoryManager:
    ...
    def _determine_priority(self, line: str) -> str:
        ...
        if any(marker in line_lower for marker in ['critical', 'urgent', 'high priority', 'major']):
            return 'high'
        elif any(marker in line_lower for marker in ['low priority', 'minor', 'nice to have']):
            return 'low'
        else:
            return 'medium'
    
    def update_memory(self, sections: Dict[str, MemorySection], 
                     updates: List[Dict[str, Any]]) -> Dict[str, MemorySection]:
    ...
    def prune_memory(self, sections: Dict[str, MemorySection], 
                    days_threshold: int = 7,
                    preserve_critical: bool = True) -> Dict[str, MemorySection]:
    ...
    def _handle_update(self, request: MemoryManagerRequest) -> MemoryManagerResponse:
        ...
        if request.updates:
            ...
            actions_taken.append({
                ...
                'items_count': len(request.updates)
            })
                    ...
                    for section in sections.values():
                        for item in section.items:
                            if (issue_content in item.content.lower() and 
                                (item.type == 'accomplishment' or 
                                 '✅' in item.content)):
                                if self.github_sync.update_issue_status(issue, 'closed'):
                                    actions_taken.append({
                                        'action': 'closed_issue',
