...
class ReadmeAgentEngine:
    ...
    def generate_readme(self,
                       ...
                       options: GenerationOptions) -> DocumentationResult:
        ...
        try:
            ...
            if template_name not in self.templates:
                template_name = project_type.value if project_type.value in self.templates else "standard"
            ...
            doc_metadata = {
                ...
                "completeness": self._calculate_completeness(sections_generated, content_requirements.sections),
                "generated_at": datetime.now().isoformat()
            }
            
            # Assess quality
            quality_metrics = self.quality_assessor.assess_quality(readme_content, sections_generated)
            
            # Generate analysis
            analysis = {
                "missing_sections": [
                    section for section in ["overview", "installation", "usage", "api_reference", "contributing", "license"]
    ...
    def _generate_section(self,
                         ...
                         repository_path: str) -> Tuple[str, SectionInfo]:
        ...
        if section_name in generators:
            content = generators[section_name](metadata, content_requirements, repository_path)
        else:
    ...
    def _generate_usage_section(self,
                               metadata: ProjectMetadata,
                               content_requirements: ContentRequirements,
                               repository_path: str) -> str:
        ...
        if "Python" in metadata.languages:
            content.append("\n```python")
            content.append(f"from {metadata.name.lower().replace('-', '_')} import main_function")
            content.append("")
            ...
            content.append("\n```javascript")
            content.append(f"const {metadata.name.replace('-', '').title()} = require('{metadata.name.lower()}');")
            content.append("")
            content.append("// Basic usage example")
            content.append("const result = new {metadata.name.replace('-', '').title()}();")
    ...
    def _generate_contributing_section(self,
                                      metadata: ProjectMetadata,
                                      content_requirements: ContentRequirements,
                                      repository_path: str) -> str:
    ...
    def _generate_license_section(self,
                                 metadata: ProjectMetadata,
                                 content_requirements: ContentRequirements,
                                 repository_path: str) -> str:
    ...
    def _generate_features_section(self,
                                  metadata: ProjectMetadata,
                                  content_requirements: ContentRequirements,
                                  repository_path: str) -> str:
    ...
    def _generate_generic_section(self, section_name: str, metadata: ProjectMetadata) -> str:
    ...
    def _generate_badges(self, metadata: ProjectMetadata, repository_path: str) -> List[Badge]:
    ...
    def _apply_template(self,
        ...
        variables = {
            ...
            "BRIEF_DESCRIPTION": metadata.description or f"A {metadata.languages[0] if metadata.languages else ''} project",
            "PROJECT_DESCRIPTION": metadata.description or f"Description for {metadata.name}",
            "FEATURE_LIST": sections.get("features", "- Feature 1\n- Feature 2"),
            "INSTALLATION_INSTRUCTIONS": sections.get("installation", "Installation instructions"),
            "USAGE_EXAMPLES": sections.get("usage", "Usage examples"),
            "API_DOCUMENTATION": sections.get("api_reference", "API documentation"),
            "CONTRIBUTING_GUIDELINES": sections.get("contributing", "Contributing guidelines"),
            ...
            "TROUBLESHOOTING_GUIDE": sections.get("troubleshooting", "Troubleshooting guide")
        }
...
```"""
    ...
    def _calculate_completeness(self, sections_generated: List[SectionInfo], requested_sections: List[str]) -> float:
    ...
    def _generate_improvement_suggestions(self,
                                        sections_generated: List[SectionInfo],
                                        quality_metrics: QualityMetrics) -> List[ImprovementSuggestion]:
    ...
    def analyze_readme(self, readme_path: str, analysis_depth: str = "detailed") -> DocumentationResult:
        ...
        try:
            ...
            analysis = {
                ...
                "link_analysis": self._analyze_links(content) if analysis_depth == "comprehensive" else {}
            }
    ...
    def _identify_missing_sections(self, existing_sections: Dict[str, str]) -> List[str]:
        """Identify missing standard sections."""
        standard_sections = [
            "installation", "usage", "api_reference", "contributing",
            "license", "features", "configuration"
        ]
    ...
    def _handle_analyze_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        analysis_depth = request_data.get("analysis_options", {}).get("depth", "detailed")
    ...
    def _handle_enhance_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "success": False,
            "error": "Enhance operation not yet implemented"
        }
    ...
    def _handle_validate_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "success": False,
            "error": "Validate operation not yet implemented"
        }
...
class ContentDiscoverer:
    ...
    def discover_project_info(self, repository_path: str, project_name: str) -> ProjectMetadata:
    ...
    def _parse_pyproject_toml(self, pyproject_toml: Path, metadata: ProjectMetadata):
        ...
        try:
            ...
            try:
                ...
                logging.warning("Cannot parse pyproject.toml: tomllib/tomli not available")
                return
    ...
    def _parse_requirements_txt(self, requirements_txt: Path, metadata: ProjectMetadata):
...
class QualityAssessor:
    ...
    def assess_quality(self, content: str, sections: List[SectionInfo]) -> QualityMetrics:
...
def main():
    ...
    if response["success"]:
        ...
        print(f"Quality score: {response['analysis']['quality_metrics']['overall_score']:.1f}")
