...
class WorktreeEnvironmentSetup:
    ...
    async def setup_environment(self, worktree_path: str, requirements: WorktreeRequirements) -> Dict[str, Any]:
    ...
    async def _ensure_uv_available(self) -> bool:
        ...
        try:
            ...
            self.logger.error("UV not available - install from https://docs.astral.sh/uv/")
            return False
            ...
            if Path(".venv").exists():
                for tool in tools:
                    ...
                    if result.returncode != 0:
                        self.logger.warning(f"Failed to install {tool} via UV: {result.stderr}")
    ...
    async def _setup_git_config(self):
        ...
        try:
            ...
            if result.returncode != 0:
                subprocess.run([
                    "git", "config", "user.name", "Gadugi Agent"
                ], check=True)
                
                subprocess.run([
                    "git", "config", "user.email", "gadugi@example.com"
                ], check=True)
...
class WorktreeHealthMonitor:
    ...
    async def _check_uv_health(self, worktree_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            health["python_version"] = result.stdout.strip() if result.returncode == 0 else "unknown"
...
class WorktreeCleanupManager:
    ...
    async def cleanup_worktrees(
        ...
        self.logger.info(f"Starting cleanup with policy '{policy}', retention {retention_days} days")
        ...
        for worktree in to_cleanup:
            try:
                if dry_run:
                    self.logger.info(f"DRY RUN: Would remove worktree {worktree.task_id}")
        ...
        self.logger.info(f"Cleanup complete: removed {result.removed_count}, "
        ...
        for worktree in worktrees:
            ...
            if worktree.created_at < cutoff_date:
                if policy == "all_old":
                    ...
                    # Check if inactive
                    inactive_hours = (datetime.now() - worktree.last_accessed).total_seconds() / 3600
            ...
            if should_cleanup and self.config["preserve_uncommitted"]:
                ...
                if has_uncommitted:
                    self.logger.info(f"Preserving {worktree.task_id} - has uncommitted changes")
                    should_cleanup = False
            
            # Preserve active worktrees
            if should_cleanup:
                preserve_hours = self.config["preserve_active_days"] * 24
                hours_since_access = (datetime.now() - worktree.last_accessed).total_seconds() / 3600
        ...
        try:
            ...
            if self.config["preserve_uncommitted"]:
                if await self._has_uncommitted_changes(worktree):
                    self.logger.warning(f"Skipping removal of {worktree.task_id} - uncommitted changes")
                    return False
            ...
            if result.returncode != 0:
                ...
                if Path(worktree.worktree_path).exists():
                    ...
                    self.logger.info(f"Manually removed directory {worktree.worktree_path}")
...
class WorktreeManagerEngine:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.environment_setup = WorktreeEnvironmentSetup(self.config.get("environment"))
        ...
        try:
            if self.registry_path.exists():
                ...
                for task_id, wt_data in data.items():
                    ...
                    wt_data["created_at"] = datetime.fromisoformat(wt_data["created_at"])
                    wt_data["last_accessed"] = datetime.fromisoformat(wt_data["last_accessed"])
        ...
        try:
            ...
            return WorktreeResult(
                ...
                metadata={
                    ...
                    "estimated_cleanup": (metadata.created_at + timedelta(
    ...
    async def _create_git_worktree(self, metadata: WorktreeMetadata) -> bool:
        ...
        try:
            ...
            subprocess.run([
                "git", "worktree", "add", "-b", metadata.branch_name,
                metadata.worktree_path, f"origin/{metadata.base_branch}"
            ], check=True, capture_output=True)
    ...
    def _create_result(self, metadata: WorktreeMetadata, error_message: str = None) -> WorktreeResult:
    ...
    async def health_check(self, task_id: str = None) -> Dict[str, Any]:
        ...
        if task_id:
            ...
            results = {}
            for task_id, metadata in self.worktrees.items():
                results[task_id] = await self.health_monitor.check_worktree_health(metadata)
            ...
            return {
                "summary": {
                    ...
                    "disk_usage_mb": sum(m.disk_usage_mb for m in self.worktrees.values())
                },
                "worktrees": results
            }
...
async def main():
    ...
    create_parser.add_argument("--uv-project", action="store_true", help="Set up UV project")
    ...
    cleanup_parser.add_argument("--policy", default="completed_tasks", 
    ...
    health_parser.add_argument("--format", choices=["summary", "detailed"], default="summary")
    ...
    if args.command == "create":
        ...
        if args.format == "json":
            ...
            # Table format
            print(f"{'Task ID':<20} {'Status':<10} {'Branch':<30} {'Age':<10} {'Size':<10}")
            print("-" * 80)
            for wt in worktrees:
                age_days = (datetime.now() - wt.created_at).days
                print(f"{wt.task_id:<20} {wt.status.value:<10} {wt.branch_name:<30} "
