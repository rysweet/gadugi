...
class AgentValidator:
    ...
    def validate_agent_file(self, filepath: Path) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            for field in self.REQUIRED_FIELDS:
                ...
                elif field in ['name', 'description'] and not str(frontmatter[field]).strip():
            ...
            if 'version' in frontmatter and frontmatter['version']:
                if not self.validate_semver(frontmatter['version']):
                    errors.append(f"Invalid version format: '{frontmatter['version']}' (expected semver like 1.0.0)")
            ...
            if 'model' not in frontmatter:
                self.warnings.append(f"{filepath}: Consider adding 'model' field (e.g., 'model: inherit')")
    ...
    def validate_all(self) -> int:
        ...
        if failed_files:
            ...
            print("\n💡 Common fixes:")
            print("   - Ensure frontmatter is between --- markers at the start of the file")
