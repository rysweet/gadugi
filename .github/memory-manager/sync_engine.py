...
from memory_parser import MemoryParser, MemoryDocument, Task, TaskStatus, TaskPriority
...
class SyncEngine:
    ...
    def sync(self, direction: Optional[SyncDirection] = None) -> SyncResult:
        ...
        try:
            ...
            print(f"Created: {result.created_issues}, Updated: {result.updated_issues}, "
                  f"Closed: {result.closed_issues}, Conflicts: {len(result.conflicts)}")
        
        return result
    
    def _sync_memory_to_github(self, memory_doc: MemoryDocument, 
                              github_issues: List[GitHubIssue], result: SyncResult):
        """Sync Memory.md tasks to GitHub issues"""
        # Create mapping of existing issues
        issue_map = {issue.memory_task_id: issue for issue in github_issues 
            ...
            try:
                if task.id in issue_map:
                    ...
                    if (task.status == TaskStatus.COMPLETED and 
                        ...
                        self.config.auto_close_completed):
    ...
    def _sync_github_to_memory(self, memory_doc: MemoryDocument, 
                              github_issues: List[GitHubIssue], result: SyncResult):
        ...
        for issue in github_issues:
            if issue.memory_task_id:
                ...
                if task:
                    ...
                    expected_status = TaskStatus.COMPLETED if issue.state == 'closed' else TaskStatus.PENDING
    ...
    def _sync_bidirectional(self, memory_doc: MemoryDocument, 
                           github_issues: List[GitHubIssue], result: SyncResult):
