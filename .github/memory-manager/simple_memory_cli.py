    ...
    if args.json:
        ...
        if args.section:
            ...
            for update in memory_data['all_updates'][-5:]:  # Show last 5
                print(f"  [{update.get('section', 'unknown')}] {update.get('timestamp', 'unknown')} - {update.get('agent', 'unknown')}")
        ...
        if results['success']:
            ...
            if results['total_results'] == 0:
                ...
                for i, result in enumerate(results['results'], 1):
                    print(f"{i}. [{result.get('section', 'unknown')}] {result.get('timestamp', 'unknown')}")
...
Examples:
  ...
  python simple_memory_cli.py search "bug fix" --section completed-tasks
    ...
    update_parser.add_argument('--section', required=True, 
                              choices=['current-goals', 'completed-tasks', 'important-context', 'next-steps', 'reflections'],
                              help='Memory section')
    update_parser.add_argument('--agent', required=True, help='Agent name adding the memory')
    update_parser.add_argument('--priority', choices=['high', 'medium', 'low'], default='medium', help='Priority level')
    update_parser.add_argument('--related', help='Comma-separated list of related issues/PRs (e.g., "#123,#456")')
