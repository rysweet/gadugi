...
class PruningConfig:
    ...
    section_rules: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
...
class IssueCreationConfig:
    ...
    # Labels by priority
    priority_label_map: Dict[str, str] = field(default_factory=lambda: {
        "high": "priority:high",
...
class ContentRules:
    ...
    required_sections: List[str] = field(default_factory=lambda: [
        "Current Goals", "Recent Accomplishments", "Next Steps"
    ])
    optional_sections: List[str] = field(default_factory=lambda: [
        "Completed Tasks", "Reflections", "Important Context",
        "Code Review Summary"
    ])
    ...
    def load_from_file(cls, file_path: str) -> 'MemoryManagerConfig':
        ...
        if 'sync' in data:
            ...
            if 'conflict_resolution' in sync_data:
                sync_data['conflict_resolution'] = ConflictResolution(sync_data['conflict_resolution'])
        ...
        # Update other fields
        for field_name in ['memory_file_path', 'config_file_path', 'state_directory',
                          'backup_directory', 'enabled', 'version']:
...
class ConfigManager:
    ...
    def save_config(self, config: MemoryManagerConfig = None):
        ...
        config_file = self._find_config_file() or (self.repo_path / self.DEFAULT_CONFIG_LOCATIONS[0])
