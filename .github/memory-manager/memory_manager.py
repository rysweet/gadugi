...
from memory_parser import MemoryParser, MemoryDocument, TaskStatus
from github_integration import GitHubIntegration
from sync_engine import SyncEngine, SyncDirection, ConflictResolution
...
class MemoryManager:
        ...
        try:
            ...
            return {
                "memory_file": {
                    ...
                    "last_updated": memory_doc.last_updated.isoformat() if memory_doc.last_updated else None,
                ...
                "config": {
                    "sync_direction": self.config.sync.direction.value if hasattr(self.config.sync.direction, 'value') else str(self.config.sync.direction),
                    "auto_create_issues": self.config.sync.auto_create_issues,
                    "conflict_resolution": self.config.sync.conflict_resolution.value if hasattr(self.config.sync.conflict_resolution, 'value') else str(self.config.sync.conflict_resolution)
                }
            }
    ...
    def resolve_conflict(self, conflict_id: str, resolution: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": success, "conflict_id": conflict_id, "resolution": resolution}
        except Exception as e:
            return {"success": False, "error": str(e)}
...
def main():
    ...
    # Prune command
    prune_parser = subparsers.add_parser("prune", help="Prune old entries from Memory.md")
    prune_parser.add_argument("--dry-run", action="store_true", help="Preview changes without applying")
    
    # Create issues command
    create_parser = subparsers.add_parser("create-issues", help="Create GitHub issues for Memory.md tasks")
    ...
    # Conflicts command
    conflicts_parser = subparsers.add_parser("conflicts", help="List synchronization conflicts")
    ...
    # Init command
    init_parser = subparsers.add_parser("init", help="Initialize Memory Manager configuration")
