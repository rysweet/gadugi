...
class TestAgentManagerHookSetup(unittest.TestCase):
    ...
    def test_hook_setup_preserves_existing_settings(self):
        ...
        self.assertEqual(
            updated_settings["permissions"], existing_settings["permissions"]
        )
        self.assertIn("other_config", updated_settings)
        self.assertEqual(
            updated_settings["other_config"], existing_settings["other_config"]
        )
    ...
    def test_hook_command_is_shell_compatible(self):
        ...
        self.assertNotIn(
            "/agent:", command, "Hook command should not use /agent: syntax"
        )

        # Verify it's a shell script execution
        self.assertIn(
            "check-agent-updates.sh",
            command,
        ...
        self.assertTrue(script_path.exists(), f"Script not created at {script_path}")
    ...
    def test_invalid_json_handling(self):
        ...
        self.assertTrue(
            valid_json, "Settings file should contain valid JSON after recovery"
        )
    ...
    def test_script_creation(self):
        ...
        self.assertTrue(
            update_script.exists(), f"Update script not created at {update_script}"
        )

        # Check that script is executable
        self.assertTrue(
            os.access(update_script, os.X_OK), "Update script is not executable"
        )

        # Check script content
        content = update_script.read_text()
        self.assertIn("#!/bin/sh", content, "Script missing shebang")
        self.assertIn("Agent Manager", content, "Script missing expected content")
