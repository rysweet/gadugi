...
def show_enhanced_dashboard():
    ...
    while True:
        try:
            ...
            status_files = list(
                Path(".gadugi/monitoring/").glob("orchestration-*_status.json")
            )
            if not status_files:
            ...
            print(
                f"📊 STATUS: {completed}/{total} complete | 🟢 {running} running | 🟡 {queued} queued"
            )
            ...
            for proc in data["active_processes"]:
                ...
                print(
                    f"{status_emoji} {proc['task_id']:8} | {proc['status']:8} | {runtime_mins:2}m {runtime_secs:2}s"
                )
