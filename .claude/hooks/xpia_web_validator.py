...
def validate_web_operation(hook_data):
    ...
    try:
        ...
        if event_type == 'PreToolUse':
            ...
            if tool_name == 'WebFetch':
                ...
                url_result = xpia_agent.validate_user_input(
                    url,
                    user_context='web_fetch_url'
                )
                
                # Validate prompt
                prompt_result = xpia_agent.validate_user_input(
                    prompt,
                    user_context='web_fetch_prompt'
                )
                ...
                if not url_result.is_safe:
                    return {
                        ...
                        'block': True,
                        'threats': [t['description'] for t in url_result.threats_detected]
                    }
                ...
                if not prompt_result.is_safe:
                    return {
                        ...
                        'block': True,
                        'threats': [t['description'] for t in prompt_result.threats_detected]
                    }
        ...
        return {
            'warning': f'XPIA validation error: {str(e)}',
            'allow': True
        }
...
def main():
    ...
    try:
        ...
        if result.get('block'):
            print(json.dumps({
                'error': result.get('error', 'XPIA Security: Operation blocked'),
        ...
        if 'modifiedToolInput' in result or 'modifiedToolOutput' in result:
            print(json.dumps({
                ...
                'message': result.get('warning') or result.get('info', 'XPIA: Content modified')
            }))
