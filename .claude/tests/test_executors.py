...
class TestCodeExecutor(unittest.TestCase):
    ...
    def test_write_file(self):
        ...
        result = self.executor.execute(
            {"action": "write", "file_path": str(file_path), "content": content}
        )
    ...
    def test_write_existing_file_fails(self):
        ...
        result = self.executor.execute(
            {"action": "write", "file_path": str(file_path), "content": "new content"}
        )
    ...
    def test_read_file(self):
        ...
        result = self.executor.execute({"action": "read", "file_path": str(file_path)})
        ...
        for line_num, line in enumerate(content.split("\n"), 1):
            ...
            for pattern in delegation_patterns:
                ...
                if re.search(pattern, line, re.IGNORECASE):
                    self.fail(
                        f"Code executor line {line_num} contains delegation pattern: {pattern}"
                    )
...
class TestGitHubExecutor(unittest.TestCase):
    ...
    def test_never_auto_merges(self, mock_run):
        ...
        result = self.executor.execute({"operation": "merge_pr", "pr_number": 123})

        self.assertTrue(result["success"])

        # But there should be no automatic merge logic
        executor_code = (
            Path(__file__).parent.parent / "executors" / "github_executor.py"
        )
        content = executor_code.read_text()
...
class TestWorkTreeExecutor(unittest.TestCase):
    ...
    def test_create_worktree(self, mock_mkdir, mock_run):
        ...
        worktree_add_called = any(
            "worktree" in str(call) and "add" in str(call) for call in calls
        )
    ...
    def test_list_worktrees(self, mock_run):
        ...
        result = self.executor.execute({"operation": "list"})
        ...
        for executor_file in executor_dir.glob("*_executor.py"):
            ...
            for line_num, line in enumerate(content.split("\n"), 1):
                ...
                for pattern in delegation_patterns:
                    ...
                    if re.search(pattern, line, re.IGNORECASE):
                        self.fail(
                            f"{executor_file.name} line {line_num} contains delegation pattern: {pattern}"
                        )
...
class TestOrchestrationInterface(unittest.TestCase):
    ...
    def test_all_executors_registered(self):
        ...
        for executor in required_executors:
            self.assertIn(
                executor, registered, f"Required executor '{executor}' not registered"
            )
