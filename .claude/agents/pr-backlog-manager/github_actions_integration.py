...
class GitHubActionsIntegration:
    ...
    def __init__(self, pr_backlog_manager):
        ...
        logger.info(f"Initialized GitHub Actions integration - Event: {self.github_context.event_type.value}")
...
def main():
    ...
    try:
        ...
        if result['success']:
            print(f"✅ PR Backlog Manager completed successfully")
            if result['processing_mode'] == 'single_pr':
                assessment = result['results']['assessment']
                print(f"PR #{result['results']['pr_number']}: {assessment['status']} "
                      f"(Score: {assessment['readiness_score']:.1f}%)")
            else:
                metrics = result['results']['metrics']
                print(f"Processed {metrics['total_prs']} PRs: "
