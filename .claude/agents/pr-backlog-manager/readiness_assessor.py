...
class ConflictAssessment:
    ...
    def is_blocking(self) -> bool:
        ...
        return self.has_conflicts and self.complexity in [ConflictComplexity.MEDIUM, ConflictComplexity.HIGH]
...
class ReviewAssessment:
    ...
    def is_review_complete(self) -> bool:
        ...
        return (
            self.has_approved_review and 
            not self.pending_requests and 
            not self.requested_changes and
            self.ai_review_complete
        )
...
class MetadataAssessment:
    ...
    def is_complete(self) -> bool:
        ...
        return (
            self.has_conventional_title and
            self.has_description and
            self.has_appropriate_labels
        )
...
class ReadinessAssessor:
    ...
    def __init__(self, github_ops):
        ...
        self.config = {
            ...
            'retriable_failure_patterns': [
                'timeout', 'network', 'rate limit', 'temporary'
            ]
        }
    ...
    def _assess_conflict_complexity(self, affected_files: List[str]) -> ConflictComplexity:
        ...
        critical_files = [
            ...
            if any(pattern in f.lower() for pattern in [
                'config', 'package.json', 'requirements.txt', 'dockerfile'
            ])
        ]
    ...
    def assess_ci_status(self, pr_details: Dict[str, Any]) -> CIAssessment:
        ...
        try:
            ...
            if checks:
                timestamps = [
                    check.get('updated_at') for check in checks 
    ...
    def _calculate_review_coverage(self, pr_details: Dict[str, Any], reviews: List[Dict[str, Any]]) -> float:
    ...
    def assess_metadata_completeness(self, pr_details: Dict[str, Any]) -> MetadataAssessment:
        ...
        try:
            ...
            expected_label_types = ['enhancement', 'bugfix', 'documentation', 'refactor', 'test']
    ...
    def _identify_blocking_factors(self, assessments: Dict[str, Any]) -> List[str]:
        ...
        if assessments['conflicts'].has_conflicts:
            blocking_factors.append(f"Merge conflicts ({assessments['conflicts'].complexity.value})")
        ...
        if not assessments['sync'].is_up_to_date:
            blocking_factors.append(f"Branch behind by {assessments['sync'].commits_behind} commits")
        
        if not assessments['metadata'].is_complete:
            blocking_factors.append(f"Metadata incomplete ({assessments['metadata'].completeness_score:.0f}%)")
