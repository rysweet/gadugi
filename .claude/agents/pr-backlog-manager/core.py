...
class PRBacklogManager:
    ...
    def __init__(
        ...
        self.state_manager = StateManager(workspace_dir=".github/workflow-states")
    ...
    def validate_auto_approve_safety(self) -> None:
        ...
        if not os.getenv("CLAUDE_AUTO_APPROVE"):
            raise GadugiError(
                "Auto-approve not explicitly enabled", severity=ErrorSeverity.CRITICAL
            )
    ...
    def _prioritize_prs(self, prs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def priority_score(pr):
            ...
            hours_since_update = (
                datetime.now().replace(tzinfo=updated_at.tzinfo) - updated_at
            ).total_seconds() / 3600
    ...
    def _generate_resolution_actions(
        ...
        for issue in blocking_issues:
            if "merge conflicts" in issue.lower():
                actions.append(
                    f"Delegate merge conflict resolution to WorkflowMaster for PR #{pr_number}"
                )
