...
5. **Coordinate Sub-Agents**: Invoke other agents like CodeReviewer at appropriate times
...
- If prompt is missing required sections:
  - Invoke PromptWriter: `/agent:PromptWriter`
...
- Example: `feature/WorkflowManager-21`
...
1. **Check if code review already exists** (recovery case):
   ```bash
   if ! gh pr view "$PR_NUMBER" --json reviews | jq -e '.reviews | length > 0' >/dev/null; then
       echo "🚨 No review found - invoking CodeReviewer (MANDATORY)"
       MUST_INVOKE_CODE_REVIEWER=true
   else
...
2. **MANDATORY: Invoke CodeReviewer sub-agent**:
   ```bash
   echo "🚨 CRITICAL: Invoking CodeReviewer agent for PR #$PR_NUMBER"
   echo "Command: /agent:CodeReviewer"
...
3. **VERIFY review was posted** (with retries):
   ...
   if ! verify_review_posted "$PR_NUMBER"; then
       ...
       echo "🚨 ENFORCING retry of CodeReviewer invocation..."

       # Retry CodeReviewer invocation
       echo "🔄 RETRY: Invoking CodeReviewer agent again"
       /agent:CodeReviewer
...
4. **MANDATORY: Invoke CodeReviewResponseAgent**:
   ...
   echo "🚨 CRITICAL: Invoking CodeReviewResponse agent"
   echo "Command: /agent:CodeReviewResponse"
...
check_and_fix_orphaned_prs() {
    ...
    while read -r pr_info; do
        if [ -n "$pr_info" ]; then
            ...
            echo "⚡ AUTOMATICALLY FIXING: Invoking CodeReviewer for PR #$PR_NUM"
    ...
    if grep -q "\[x\] Phase 8:" "$STATE_FILE" && ! grep -q "\[x\] Phase 9:" "$STATE_FILE"; then
        ...
        if [ -n "$PR_NUM" ]; then
            ...
            if ! gh pr view "$PR_NUM" --json reviews | jq -e '.reviews | length > 0' >/dev/null; then
                ...
                echo "⚡ ENFORCING Phase 9: Invoking CodeReviewer immediately"
...
**AUTOMATIC EXECUTION**: This phase runs automatically after CodeReviewResponse completion in Phase 10.
...
1. **Check for Local Settings Changes**:
   ...
   if [ -f ".claude/settings.local.json" ]; then
       ...
       echo "🚀 AUTOMATIC: Invoking ClaudeSettingsUpdate agent"
...
```bash
# After CodeReviewResponse completion in Phase 10
...
execute_phase_11_with_error_handling() {
    ...
    if /agent:ClaudeSettingsUpdate; then
        echo "✅ Settings update completed successfully"
        complete_phase 11 "Settings Update" "verify_phase_11"
    else
...
execute_phase_12_with_error_handling() {
    ...
    if cd .github/MemoryManager && python3 memory_manager.py auto-compact 2>/dev/null; then
...
- **CodeReviewer**: Invoke for PR reviews
...
2. **Generate full task list (including review tasks)** ✅
   ...
   # - Task 9: MANDATORY CodeReviewer invocation
   # - Task 10: MANDATORY CodeReviewResponse
