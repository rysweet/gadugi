...
from shared_test_instructions import (
    SharedTestInstructions, TestResult, TestStatus, TestAnalysis
)
...
class TestWriterAgent:
    ...
    def create_tests(self, code_path: str, context: str = "", 
                    test_type: TestType = TestType.UNIT,
                    existing_fixtures: Optional[List[str]] = None) -> TestWriterResult:
        """
    ...
    def _define_test_scope(self, code_analysis: CodeAnalysis, test_type: TestType) -> Dict[str, List[str]]:
        ...
        if test_type == TestType.UNIT:
            base_scope['functionality_tests'].extend(['boundary_conditions', 'parameter_validation'])
            ...
            base_scope['security_tests'].extend(['input_validation', 'access_control', 'data_sanitization'])
    ...
    def _analyze_tdd_context(self, context: str, code_analysis: CodeAnalysis) -> Dict[str, Any]:
        ...
        if tdd_context['is_tdd_scenario']:
            ...
            if any(keyword in context.lower() for keyword in ['behavior', 'should', 'must', 'expected']):
    ...
    def _design_test_structure(self, code_analysis: CodeAnalysis, test_scope: Dict[str, List[str]]) -> TestPlan:
    ...
    def _plan_fixtures(self, test_plan: TestPlan, existing_fixtures: List[str]) -> Dict[str, List[str]]:
    ...
    def _create_test_methods(self, test_plan: TestPlan, code_analysis: CodeAnalysis, 
                           tdd_context: Dict[str, Any]) -> List[TestSpec]:
    ...
    def _create_individual_test(self, test_method: Dict[str, Any], 
                              code_analysis: CodeAnalysis,
                              tdd_context: Dict[str, Any]) -> TestSpec:
        ...
        return TestSpec(
            ...
            documentation=self._generate_test_documentation(test_method, code_analysis, tdd_context)
        )
...
"""
        ...
        deps_valid, dep_issues = self.shared_instructions.validate_dependency_management(enhanced_code)
        if not deps_valid:
            enhanced_code = self._fix_dependency_issues(enhanced_code, dep_issues)
        
        # Validate parallel safety
        parallel_safe, parallel_issues = self.shared_instructions.validate_parallel_safety(enhanced_code)
    ...
    def _validate_test_quality(self, tests_created: List[TestSpec], 
                              fixtures_created: List[FixtureSpec]) -> Dict[str, bool]:
        ...
        for fixture in fixtures_created:
            has_cleanup = 'cleanup' in fixture.cleanup_code.lower() or fixture.cleanup_code.strip() != ""
    ...
    def _document_tests(self, tests_created: List[TestSpec], 
                       code_analysis: CodeAnalysis, context: str) -> List[TestSpec]:
...
"""'''
    ...
    def _validate_complete_test_suite(self, tests_created: List[TestSpec]) -> List[str]:
        ...
        if well_documented == len(tests_created):
            ...
            validation_results.append(f"⚠️ {len(tests_created) - well_documented} tests need better documentation")
...
"""
    ...
    def _generate_generic_test(self, method_name: str, category: str, code_analysis: CodeAnalysis) -> str:
...
"""
    ...
    def _determine_fixtures_needed(self, code_analysis: CodeAnalysis, test_methods: List[Dict]) -> List[str]:
    ...
    def _determine_cleanup_requirements(self, code_analysis: CodeAnalysis) -> List[str]:
        ...
        if any('db' in dep.lower() or 'database' in dep.lower() for dep in code_analysis.dependencies):
...
"""
    ...
    def _generate_test_documentation(self, test_method: Dict[str, Any], 
                                   code_analysis: CodeAnalysis,
                                   tdd_context: Dict[str, Any]) -> str:
...
"""'''
    ...
    def _create_error_result(self, code_path: str, error_message: str) -> TestWriterResult:
        ...
        return TestWriterResult(
            ...
            tdd_alignment={'design_guidance_provided': False, 'interfaces_specified': False, 'behaviors_documented': False},
