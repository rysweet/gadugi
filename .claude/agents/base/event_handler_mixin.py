...
class EventHandlerMixin:
        ...
        try:
            ...
            async with self._event_session.post(event_url, json=event_data) as response:
                if response.status in (200, 201, 202):
                    ...
                    if (
                        self.event_config.store_in_memory
                        and self.memory
                        and event_data.get("priority") in ["high", "critical"]
                    ):
                        try:
    ...
    async def emit_stopped(
        self, output_data: Any, task_id: str, success: bool, duration_seconds: float
    ) -> bool:
    ...
    async def emit_needs_approval(
        self, command: str, description: str, risk_level: str = "medium"
    ) -> bool:
