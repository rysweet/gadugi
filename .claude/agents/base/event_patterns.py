...
class WorkflowPatterns:
    ...
    @staticmethod
    def create_dependency_completion_pattern(dependency_task_ids: List[str]) -> EventPattern:
...
class MonitoringPatterns:
    ...
    def create_health_check_pattern() -> EventPattern:
        ...
        return EventPattern(
            event_types={'system.health_check', 'agent.heartbeat'}
        )
...
class ExampleEventReactionAgent(EventSubscriber):
    ...
    async def handle_dependency_completion(self, event: AgentEvent):
        ...
        try:
            ...
            await self.emit_error("dependency_error", f"Failed to handle dependency completion: {e}")
    ...
    async def handle_error_investigation(self, event: AgentEvent):
        ...
        try:
            ...
            await self.emit_error("investigation_error", f"Failed to handle error investigation: {e}")
    ...
    async def setup_aggregated_reactions(self):
        ...
        knowledge_pattern = EventPattern(
            event_types={'knowledge.learned'}
        )
        self.subscribe_to_events(
            pattern=knowledge_pattern,
    ...
    async def trigger_knowledge_sharing_session(self, events: List[AgentEvent]):
        ...
        try:
            ...
            await self.emit_error("knowledge_session_error", f"Failed to trigger knowledge sharing: {e}")
