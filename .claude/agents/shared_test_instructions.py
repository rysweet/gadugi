...
class SharedTestInstructions:
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        self.config = config or AgentConfig(
            agent_id="shared_test_instructions",
    ...
    def validate_dependency_management(test_code: str) -> Tuple[bool, List[str]]:
        ...
        has_external = any(dep in dep_name.lower() for dep_name in dependencies 
        ...
        has_db = any(dep in dep_name.lower() for dep_name in dependencies 
                    for dep in db_deps)
    ...
    @staticmethod
    def validate_skip_justification(skip_reason: Optional[SkipReason], 
        ...
        reason_keywords = {
            ...
            SkipReason.PLATFORM_CONSTRAINT: ['platform', 'os', 'windows', 'linux', 'mac'],
            SkipReason.UPSTREAM_BUG: ['upstream', 'bug', 'issue', 'broken'],
            SkipReason.INFRASTRUCTURE_DEPENDENCY: ['infrastructure', 'service', 'external'],
        ...
        has_matching_keyword = any(keyword in justification.lower() 
    ...
    def _identify_resources(test_code: str) -> List[str]:
        ...
        resources = []
        resource_patterns = ['file', 'tempfile', 'socket', 'database', 'network', 'process']
