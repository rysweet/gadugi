...
class RecommendationEngine:
    ...
    def generate_task_assignment_recommendation(self, 
                                              matching_result: MatchingRecommendation) -> Recommendation:
        """Generate recommendation from task matching result."""
        try:
            primary_agent = matching_result.recommended_agents[0] if matching_result.recommended_agents else "N/A"
            ...
            recommendation = Recommendation(
                ...
                metadata={'task_id': matching_result.task_id, 'strategy': matching_result.assignment_strategy.value}
            )
            
            # Add alternative actions
            for alt_agent, score in matching_result.alternative_options:
                recommendation.alternative_actions.append(f"Alternative: Assign to {alt_agent} (score: {score:.2f})")
    ...
    def generate_team_formation_recommendation(self, 
                                             optimization_result: OptimizationResult) -> Recommendation:
        ...
        try:
            ...
            recommendation = Recommendation(
                ...
                metadata={'project_id': optimization_result.project_id, 'team_size': len(optimal_team.agents)}
            )
