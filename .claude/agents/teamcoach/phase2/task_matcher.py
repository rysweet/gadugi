...
class TaskAgentMatcher:
    ...
    def find_optimal_agent(self, 
        ...
        try:
            ...
            if not agent_scores:
                raise MatchingError(f"No suitable agents found for task {task_requirements.task_id}")
            ...
            self.logger.error(f"Failed to find optimal agent for task {task_requirements.task_id}: {e}")
            raise MatchingError(f"Matching failed for task {task_requirements.task_id}: {e}")
    
    def _calculate_agent_task_score(self, 
                                  agent_id: str, 
                                  task_requirements: TaskRequirements,
        ...
        try:
            ...
            strengths, concerns, recommendations = self._analyze_match_factors(
                agent_id, capability_profile, task_requirements, 
                capability_match, performance_prediction, availability_score
            )
        ...
        try:
            ...
            for domain, required_level in task_requirements.required_capabilities.items():
                if domain in capability_profile.capability_scores:
                    ...
                    level_match = min(1.0, agent_capability.proficiency_level.value / required_level.value)
            ...
            for domain, preferred_level in task_requirements.preferred_capabilities.items():
                if domain in capability_profile.capability_scores:
                    ...
                    level_match = min(1.0, agent_capability.proficiency_level.value / preferred_level.value)
                    confidence_weight = agent_capability.confidence_score
    ...
    def _predict_task_performance(self, 
                                agent_id: str,
                                task_requirements: TaskRequirements) -> float:
    ...
    def _calculate_workload_balance_score(self, 
                                        availability: AgentAvailability,
    ...
    def _calculate_task_type_similarity_adjustment(self, 
                                                 agent_id: str, 
                                                 task_type: str) -> float:
        ...
        try:
            ...
            task_results = self.task_metrics.get_agent_task_results(agent_id, start_time, end_time)
            ...
            similar_tasks = [
                ...
                if hasattr(result, 'task_type') and 
                   self._calculate_task_type_similarity(result.task_type, task_type) > 0.7
            ]
            ...
            similar_success_rate = sum(1 for task in similar_tasks if task.success) / len(similar_tasks)
    ...
    def _calculate_trend_adjustment(self, performance_data) -> float:
        ...
        try:
            if not performance_data.performance_trend or len(performance_data.performance_trend) < 2:
                return 0.0
    ...
    def _calculate_complexity_adjustment(self, performance_data, task_requirements: TaskRequirements) -> float:
    ...
    def _calculate_confidence_level(self, 
        ...
        try:
            ...
            relevant_capabilities = (
                list(task_requirements.required_capabilities.keys()) +
            ...
            if capability_confidences:
                avg_capability_confidence = sum(capability_confidences) / len(capability_confidences)
    ...
    def _calculate_task_familiarity_confidence(self, agent_id: str, task_type: str) -> float:
        ...
        try:
            ...
            similar_tasks = [
                result for result in task_results
                if hasattr(result, 'task_type') and 
                   self._calculate_task_type_similarity(result.task_type, task_type) > 0.5
            ]
    ...
    def _analyze_match_factors(self, 
                             agent_id: str,
                             capability_profile: AgentCapabilityProfile,
                             task_requirements: TaskRequirements,
                             capability_match: float,
        ...
        try:
            ...
            if capability_match < 0.6:
                ...
                for domain, required_level in task_requirements.required_capabilities.items():
                    if domain in capability_profile.capability_scores:
                        agent_level = capability_profile.capability_scores[domain].proficiency_level
    ...
    def _generate_recommendation(self, 
        ...
        try:
            ...
            if recommended_agents:
                top_scores = [agent_scores[agent_id].overall_score for agent_id in recommended_agents]
    ...
    def _enhance_recommendation_reasoning(self, 
        ...
        try:
            ...
            reasoning_parts.append(strategy_explanations.get(
            ...
            if recommendation.risk_factors:
                reasoning_parts.append(f"Risk factors identified: {len(recommendation.risk_factors)}")
            
            # Alternative options
            if recommendation.alternative_options:
                reasoning_parts.append(f"{len(recommendation.alternative_options)} alternative options available")
    ...
    def _estimate_completion_time(self, 
        ...
        try:
            ...
            performance_data = self.performance_analyzer.analyze_agent_performance(primary_agent)
            
            if performance_data.avg_execution_time > 0:
                # Adjust estimated duration based on agent performance
                agent_efficiency = min(2.0, 300.0 / performance_data.avg_execution_time)  # Baseline 5 minutes
    ...
    def _identify_risk_factors(self, 
        ...
        try:
            ...
            if task_requirements.deadline:
                ...
                if task_requirements.estimated_duration and time_to_deadline < task_requirements.estimated_duration * 1.2:
        ...
        try:
            for agent_id in agent_ids:
                ...
                if (agent_id not in self.agent_profiles_cache or 
                    (datetime.now() - self.agent_profiles_cache[agent_id].profile_generated) > timedelta(hours=24)):
