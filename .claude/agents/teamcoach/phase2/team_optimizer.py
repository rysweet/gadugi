...
from ..phase1.capability_assessment import (
    CapabilityAssessment, AgentCapabilityProfile, CapabilityDomain, ProficiencyLevel
)
...
class TeamCompositionOptimizer:
    ...
    def optimize_team_for_project(self, 
        ...
        try:
            ...
            self.logger.info(f"Optimizing team composition for project {project_requirements.project_id}")
            ...
            alternatives = sorted(
                [c for c in evaluated_compositions if c.composition_id != optimal_composition.composition_id],
            ...
            self.logger.error(f"Failed to optimize team for project {project_requirements.project_id}: {e}")
            raise OptimizationError(f"Team optimization failed: {e}")
    
    def _generate_candidate_compositions(self, 
                                       project_requirements: ProjectRequirements,
                                       available_agents: List[str]) -> List[TeamComposition]:
    ...
    def _is_feasible_composition(self, 
                               agents: List[str], 
                               project_requirements: ProjectRequirements) -> bool:
        ...
        try:
            ...
            coverage_ratio = len(covered_capabilities.intersection(required_capabilities)) / len(required_capabilities)
    ...
    def _evaluate_team_composition(self, 
        ...
        try:
            ...
            for objective in objectives:
                score = self._calculate_objective_score(composition, objective, project_requirements)
            ...
            self.logger.error(f"Failed to evaluate team composition {composition.composition_id}: {e}")
            composition.overall_score = 0.0
    
    def _calculate_capability_coverage(self, 
                                     composition: TeamComposition,
                                     project_requirements: ProjectRequirements) -> None:
        ...
        try:
            ...
            for domain, required_level in project_requirements.required_capabilities.items():
                agent_capabilities = []
                ...
                if agent_capabilities:
                    ...
                    capable_agents = sum(1 for level in agent_capabilities if level >= required_level.value * 0.8)
            ...
            # Identify gaps
            capability_gaps = [
                domain for domain, coverage in capability_coverage.items()
    ...
    def _predict_composition_performance(self, 
                                       composition: TeamComposition,
                                       project_requirements: ProjectRequirements) -> None:
        ...
        try:
            ...
            if individual_success_rates:
                ...
                team_size_factor = 1.0 - (0.05 * (len(composition.agents) - 1))  # Small penalty for coordination
            ...
            if individual_completion_times and project_requirements.task_list:
                avg_completion_time = sum(individual_completion_times) / len(individual_completion_times)
                # Assume some parallelization benefit but coordination overhead
                parallelization_factor = 0.7 + (0.3 / len(composition.agents))
                estimated_total_time = len(project_requirements.task_list) * avg_completion_time * parallelization_factor
                composition.predicted_completion_time = timedelta(seconds=estimated_total_time)
            
            # Calculate risk score
            composition.risk_score = self._calculate_team_risk_score(composition, project_requirements)
    ...
    def _assess_team_dynamics(self, composition: TeamComposition) -> None:
        ...
        try:
            ...
            composition.communication_complexity = min(1.0, communication_paths / max_comfortable_paths)
    ...
    def _calculate_workload_balance(self, composition: TeamComposition) -> float:
        ...
        try:
            ...
            workload_variance = sum((score - avg_workload) ** 2 for score in workload_scores) / len(workload_scores)
    ...
    def _calculate_team_risk_score(self, 
                                 composition: TeamComposition,
                                 project_requirements: ProjectRequirements) -> float:
        """Calculate overall risk score for the team composition."""
        try:
            risk_factors = []
            
            # Capability gap risk
            capability_gap_risk = len(composition.capability_gaps) / len(project_requirements.required_capabilities)
        ...
        try:
            if objective == OptimizationObjective.MAXIMIZE_CAPABILITY:
                ...
                if composition.capability_coverage:
                    return sum(composition.capability_coverage.values()) / len(composition.capability_coverage)
                return 0.0
                ...
                for agent_id in composition.agents:
                    if agent_id in self.agent_profiles_cache:
                        ...
                        aligned_improvements = len(set(profile.improvement_areas).intersection(
                            set(project_requirements.required_capabilities.keys())
                        ))
                        learning_score += aligned_improvements
                
                max_possible_learning = len(composition.agents) * len(project_requirements.required_capabilities)
    ...
    def _analyze_composition_factors(self, 
                                   composition: TeamComposition,
                                   project_requirements: ProjectRequirements) -> None:
        ...
        try:
            ...
            strong_capabilities = [
                domain.value for domain, coverage in composition.capability_coverage.items()
                if coverage >= 0.9
            ]
    ...
    def _calculate_optimization_confidence(self, 
        ...
        try:
            ...
            if optimal_composition.capability_coverage:
                avg_coverage = sum(optimal_composition.capability_coverage.values()) / len(optimal_composition.capability_coverage)
    ...
    def _enhance_optimization_result(self, 
        ...
        try:
            ...
            if len(optimal.agents) > 4:
                trade_offs.append("Larger team provides better coverage but increases coordination complexity")
            ...
            if len(optimal.agents) > 5:
                risk_mitigation.append("Establish clear communication protocols and coordination structure")
    ...
    def compare_team_compositions(self, 
        ...
        try:
            ...
            criteria = criteria or [
                'overall_score', 'predicted_success_rate', 'collaboration_score',
                'risk_score', 'team_size'
            ]
            ...
            for criterion in criteria:
                if criterion == 'risk_score':
                    ...
                    ranked = sorted(compositions, key=lambda c: getattr(c, criterion, 0.0), reverse=True)
