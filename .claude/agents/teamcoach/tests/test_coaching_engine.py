...
from ..phase3.coaching_engine import (
    CoachingEngine, CoachingRecommendation, TeamCoachingPlan,
    CoachingPriority, CoachingCategory
)
...
class TestCoachingEngine(unittest.TestCase):
    ...
    def test_generate_agent_coaching_capability_gaps(self):
        ...
        skill_recs = [r for r in recommendations if r.category == CoachingCategory.SKILL_DEVELOPMENT]
    ...
    def test_generate_team_coaching_plan(self):
        ...
        with patch.object(self.engine, '_analyze_team_capability_balance', 
                         return_value={'gaps': ['ai', 'ml'], 'total_domains': 10}):
            with patch.object(self.engine, '_calculate_team_collaboration_score', 
                             return_value=0.6):
                # Generate team plan
    ...
    def test_coaching_priority_ranking(self):
        ...
        sorted_recs = sorted(
            recs,
            key=lambda r: self.engine._get_priority_rank(r.priority),
            reverse=True
        )
