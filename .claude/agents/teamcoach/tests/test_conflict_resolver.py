...
from ..phase3.conflict_resolver import (
    ConflictResolver, AgentConflict, ConflictResolution,
...
class TestConflictResolver(unittest.TestCase):
    ...
    def test_detect_resource_contention(self):
        ...
        conflicts = self.resolver.detect_conflicts(
            self.agent_states, self.team_context
        )
        
        # Find resource conflicts
        resource_conflicts = [
            c for c in conflicts 
            if c.conflict_type == ConflictType.RESOURCE_CONTENTION
        ]
        ...
        db_conflicts = [
            ...
            if c.evidence.get('resource') == 'database'
        ]
    ...
    def test_detect_task_overlap(self):
        ...
        task_conflicts = [
            c for c in conflicts 
            if c.conflict_type == ConflictType.TASK_OVERLAP
        ]
    ...
    def test_detect_coordination_failures(self):
        ...
        coord_conflicts = [
            c for c in conflicts 
            if c.conflict_type == ConflictType.COORDINATION_FAILURE
        ]
    ...
    def test_detect_capability_mismatches(self):
        ...
        cap_conflicts = [
            c for c in conflicts 
            if c.conflict_type == ConflictType.CAPABILITY_MISMATCH
        ]
    ...
    def test_detect_dependency_deadlock(self):
        ...
        circular_states = {
            'agent_1': {
                'waiting_for': [{'provider': 'agent_2', 'wait_time': 1000}]
            },
            'agent_2': {
                'waiting_for': [{'provider': 'agent_3', 'wait_time': 1000}]
            },
            'agent_3': {
                'waiting_for': [{'provider': 'agent_1', 'wait_time': 1000}]
            }
        }
        
        conflicts = self.resolver.detect_conflicts(
            circular_states, self.team_context
        )
        
        # Find deadlock conflicts
        deadlock_conflicts = [
            c for c in conflicts 
            if c.conflict_type == ConflictType.DEPENDENCY_DEADLOCK
        ]
    ...
    def test_resolve_conflict_task_overlap(self):
        ...
        remove_actions = [
            a for a in resolution.actions 
            if a['type'] == 'remove_task'
        ]
    ...
    def test_implement_resolution(self):
        ...
        result = self.resolver.implement_resolution(
            conflict, resolution, test_states
        )
    ...
    def test_prevention_recommendations(self):
        ...
        for i in range(10):
            self.resolver.conflict_patterns[f'resource_contention_high'] = 10
