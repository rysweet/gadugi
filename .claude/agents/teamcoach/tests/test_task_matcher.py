...
from ..phase2.task_matcher import (
    TaskAgentMatcher, TaskRequirements, AgentAvailability, MatchingScore,
    MatchingRecommendation, MatchingStrategy, TaskPriority, TaskUrgency,
    MatchingError
)
from ..phase1.capability_assessment import (
    CapabilityDomain, ProficiencyLevel, AgentCapabilityProfile, CapabilityScore
)
...
class TestTaskAgentMatcher(unittest.TestCase):
    ...
    def test_find_optimal_agent_success(self):
        ...
        # Mock dependencies
        self.mock_capability_assessment.assess_agent_capabilities.return_value = self.mock_capability_profile
        ...
        self.mock_performance_analyzer.analyze_agent_performance.return_value = mock_performance_data
        
        # Mock task metrics for availability
        self.mock_task_metrics.get_agent_active_tasks.return_value = []
        
        # Execute matching
        with patch.object(self.matcher, '_get_agent_availability', return_value=self.mock_availability):
    ...
    def test_calculate_availability_score(self):
        ...
        score = self.matcher._calculate_availability_score(good_availability, self.task_requirements)
        ...
        score_poor = self.matcher._calculate_availability_score(poor_availability, self.task_requirements)
    ...
    def test_batch_match_tasks(self):
        ...
        task_list = [
            TaskRequirements(
                ...
                required_capabilities={CapabilityDomain.CODE_GENERATION: ProficiencyLevel.INTERMEDIATE}
            )
            for i in range(3)
        ]
