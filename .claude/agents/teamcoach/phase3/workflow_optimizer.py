...
class WorkflowOptimizer:
    ...
    def analyze_workflow(self,
                        ...
                        task_history: List[Dict[str, Any]]) -> WorkflowAnalysis:
        """
        ...
        projected_metrics = self._project_improvements(
    ...
    def _calculate_workflow_metrics(self,
                                  workflow_data: Dict[str, Any],
                                  agent_states: Dict[str, Dict[str, Any]],
                                  task_history: List[Dict[str, Any]]) -> WorkflowMetrics:
    ...
    def _detect_bottlenecks(self,
                          ...
                          metrics: WorkflowMetrics) -> List[Bottleneck]:
    ...
    def _generate_optimizations(self,
                              workflow_data: Dict[str, Any],
                              bottlenecks: List[Bottleneck],
                              metrics: WorkflowMetrics) -> List[WorkflowOptimization]:
    ...
    def _detect_resource_bottlenecks(self,
                                   workflow_data: Dict[str, Any],
                                   agent_states: Dict[str, Dict[str, Any]],
                                   task_history: List[Dict[str, Any]]) -> List[Bottleneck]:
        ...
        for resource, usage in resource_usage.items():
            ...
            if utilization > self.bottleneck_thresholds['resource_utilization']:
                ...
                bottleneck = Bottleneck(
                    ...
                    affected_agents=[t.get('agent_id') for t in task_history if resource in t.get('resources_used', [])],
                    affected_tasks=[t.get('task_id') for t in task_history if resource in t.get('resources_used', [])],
                    description=f"Resource '{resource}' is overutilized ({utilization:.1%})",
                    evidence={
                        'resource': resource,
                        'utilization': utilization,
                        'total_wait_time': resource_waits[resource],
                        'affected_task_count': len([t for t in task_history if resource in t.get('resources_used', [])])
                    },
    ...
    def _detect_skill_bottlenecks(self,
                                workflow_data: Dict[str, Any],
                                agent_states: Dict[str, Dict[str, Any]],
                                task_history: List[Dict[str, Any]]) -> List[Bottleneck]:
        ...
        for skill, demand in skill_demand.items():
            ...
            if supply == 0 or demand / supply > 3:  # High demand/supply ratio
                ...
                bottleneck = Bottleneck(
                    ...
                    affected_tasks=[t.get('task_id') for t in task_history if skill in t.get('required_skills', [])],
    ...
    def _detect_dependency_bottlenecks(self,
        ...
        if critical_path:
            ...
            workflow_duration = max(t.get('end_time', 0) for t in task_history) - min(t.get('start_time', 0) for t in task_history)
            ...
            if total_duration / workflow_duration > 0.8:  # Critical path dominates
                bottleneck = Bottleneck(
                    ...
                    affected_agents=list(set(t.get('agent_id') for t in task_history if t.get('task_id') in critical_path)),
                    ...
                    evidence={
                        ...
                        'path_percentage': total_duration / workflow_duration if workflow_duration > 0 else 0
                    },
                    detected_at=datetime.utcnow()
                )
                bottlenecks.append(bottleneck)
        ...
        for pair, delays in communication_delays.items():
            ...
            if avg_delay > self.bottleneck_thresholds['communication_delay']:
                bottleneck = Bottleneck(
                    ...
                    affected_agents=list(pair) if pair[0] != 'general' else list(agent_states.keys()),
                    affected_tasks=[t.get('task_id') for t in task_history if t.get('communication_delay', 0) > self.bottleneck_thresholds['communication_delay']],
    ...
    def _detect_process_bottlenecks(self,
                                  workflow_data: Dict[str, Any],
                                  task_history: List[Dict[str, Any]],
                                  metrics: WorkflowMetrics) -> List[Bottleneck]:
        ...
        if metrics.efficiency_ratio < 0.5:
            bottleneck = Bottleneck(
                ...
                evidence={
                    ...
                    'wait_time_ratio': metrics.wait_time / metrics.total_duration if metrics.total_duration > 0 else 0,
    ...
    def _generate_resource_optimization(self,
                                      bottleneck: Bottleneck,
                                      workflow_data: Dict[str, Any]) -> Optional[WorkflowOptimization]:
        """Generate optimization for resource constraints."""
        resource = bottleneck.evidence.get('resource')
        utilization = bottleneck.evidence.get('utilization', 0)
        ...
        optimization = WorkflowOptimization(
            ...
            prerequisites=["Resource usage audit", "Alternative resource identification"],
            risks=["Temporary disruption during reallocation", "Cost of additional resources"]
        )
        
        return optimization
    
    def _generate_parallelization_optimization(self,
                                             bottleneck: Bottleneck,
                                             workflow_data: Dict[str, Any]) -> Optional[WorkflowOptimization]:
    ...
    def _generate_process_optimization(self,
                                     bottleneck: Bottleneck,
                                     workflow_data: Dict[str, Any]) -> Optional[WorkflowOptimization]:
        ...
        optimization = WorkflowOptimization(
            ...
            risks=["Initial slowdown during implementation", "Resistance to process change"]
        )
        
        return optimization
    
    def _generate_skill_optimization(self,
                                   bottleneck: Bottleneck,
                                   workflow_data: Dict[str, Any]) -> Optional[WorkflowOptimization]:
        ...
        optimization = WorkflowOptimization(
            ...
            risks=["Time investment for training", "Skill development may take longer than expected"]
        )
        
        return optimization
    
    def _generate_communication_optimization(self,
                                           bottleneck: Bottleneck,
                                           workflow_data: Dict[str, Any]) -> Optional[WorkflowOptimization]:
        """Generate optimization for communication issues."""
        avg_delay = bottleneck.evidence.get('average_delay', 0)
    ...
    def _generate_parallelization_improvement(self,
                                            workflow_data: Dict[str, Any],
                                            metrics: WorkflowMetrics) -> Optional[WorkflowOptimization]:
        ...
        optimization = WorkflowOptimization(
            ...
            expected_improvement=(0.8 - current_efficiency) * 50 if current_efficiency < 0.8 else 10,
            implementation_steps=[
    ...
    def _prioritize_optimizations(self,
                                optimizations: List[WorkflowOptimization]) -> List[WorkflowOptimization]:
    ...
    def _project_improvements(self,
                            current_metrics: WorkflowMetrics,
                            optimizations: List[WorkflowOptimization]) -> WorkflowMetrics:
