...
from ...shared.interfaces import AgentConfig, PerformanceMetrics
...
from .metrics_collector import MetricsCollector, AggregatedMetric
...
class ReportingSystem:
    ...
    def _generate_agent_performance_report(self, report: GeneratedReport, config: ReportConfig) -> None:
        ...
        try:
            for agent_id in config.agents:
                ...
                report.metrics_included.extend([
                    'success_rate', 'execution_time', 'resource_efficiency', 'quality_score'
                ])
    ...
    def _generate_capability_analysis_report(self, report: GeneratedReport, config: ReportConfig) -> None:
    ...
    def _generate_trend_analysis_report(self, report: GeneratedReport, config: ReportConfig) -> None:
    ...
    def _generate_comparative_analysis_report(self, report: GeneratedReport, config: ReportConfig) -> None:
    ...
    def _generate_executive_summary_report(self, report: GeneratedReport, config: ReportConfig) -> None:
        ...
        try:
            ...
            if all_success_rates:
                summary_data['key_metrics']['avg_success_rate'] = sum(all_success_rates) / len(all_success_rates)
            if all_execution_times:
                summary_data['key_metrics']['avg_execution_time'] = sum(all_execution_times) / len(all_execution_times)
            if all_quality_scores:
                summary_data['key_metrics']['avg_quality_score'] = sum(all_quality_scores) / len(all_quality_scores)
...
"""
    ...
    def _format_team_overview(self, team_aggregates: Dict[str, Any], agent_summaries: List[Dict[str, Any]]) -> str:
...
"""
        if performance_data.performance_trend:
            trend_direction = "improving" if performance_data.performance_trend[-1] > performance_data.performance_trend[0] else "declining"
    ...
    def _format_comparative_analysis(self, agent_performances: Dict[str, AgentPerformanceData]) -> str:
    ...
    def _format_executive_summary_content(self, summary_data: Dict[str, Any]) -> str:
        ...
        content += "\n### Key Recommendations\n"
        unique_recommendations = list(set(summary_data['recommendations']))[:5]  # Top 5 unique recommendations
    ...
    def _generate_performance_charts(self, performance_data: AgentPerformanceData) -> List[str]:
        ...
        try:
            ...
            if performance_data.performance_trend and len(performance_data.performance_trend) > 1:
                ...
                ax.plot(x, performance_data.performance_trend, marker='o', linewidth=2, markersize=6)
    ...
    def _generate_team_charts(self, team_aggregates: Dict[str, Any], agent_summaries: List[Dict[str, Any]]) -> List[str]:
    ...
    def _generate_capability_charts(self, capability_profile: AgentCapabilityProfile) -> List[str]:
        ...
        try:
            ...
            if capability_profile.capability_scores:
                ...
                capabilities = list(capability_profile.capability_scores.keys())[:8]  # Limit to 8 for readability
                proficiency_values = [
                ...
                ax.plot(angles, proficiency_values, 'o-', linewidth=2, label='Proficiency', color='blue')
                ax.fill(angles, proficiency_values, alpha=0.25, color='blue')
                ax.plot(angles, confidence_values, 'o-', linewidth=2, label='Confidence', color='red')
        ...
        try:
            if performance_data.performance_trend and len(performance_data.performance_trend) > 1:
                ...
                if len(x) > 2:
                    z = np.polyfit(x, y, 1)
                    p = np.poly1d(z)
                    ax.plot(x, p(x), "--", alpha=0.8, color='red', linewidth=2, label='Trend Line')
                ...
                if len(y) > 0:
                    ...
                    ax.annotate(f'Peak: {max(y):.2f}', 
                               xy=(max_idx, max(y)), xytext=(max_idx, max(y) + 0.1),
                               arrowprops=dict(arrowstyle='->', color='green'),
                               ha='center')
                    
                    ax.annotate(f'Low: {min(y):.2f}', 
                               xy=(min_idx, min(y)), xytext=(min_idx, min(y) - 0.1),
    ...
    def _generate_comparison_charts(self, agent_performances: Dict[str, AgentPerformanceData]) -> List[str]:
