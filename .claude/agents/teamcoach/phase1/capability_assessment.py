...
class CapabilityAssessment:
    ...
    def _assess_domain_capability(self, 
                                domain: CapabilityDomain,
                                tasks: List[TaskResult],
                                agent_id: str) -> CapabilityScore:
        ...
        try:
            ...
            execution_times = [task.execution_time for task in tasks if task.execution_time is not None]
    ...
    def _group_tasks_by_domain(self, tasks: List[TaskResult]) -> Dict[CapabilityDomain, List[TaskResult]]:
    ...
    def _determine_task_domain(self, task: TaskResult) -> Optional[CapabilityDomain]:
        ...
        domain_keywords = {
            CapabilityDomain.CODE_GENERATION: ['implement', 'create', 'build', 'develop', 'code'],
            ...
            CapabilityDomain.ARCHITECTURE: ['design', 'architecture', 'structure', 'pattern'],
            CapabilityDomain.DEBUGGING: ['debug', 'fix', 'resolve', 'troubleshoot'],
            CapabilityDomain.INTEGRATION: ['integrate', 'merge', 'combine', 'connect'],
            CapabilityDomain.PERFORMANCE_OPTIMIZATION: ['optimize', 'performance', 'speed', 'efficiency'],
            CapabilityDomain.SECURITY: ['security', 'secure', 'auth', 'permission'],
            CapabilityDomain.DATA_ANALYSIS: ['analyze', 'data', 'metrics', 'report'],
            CapabilityDomain.PROJECT_MANAGEMENT: ['manage', 'coordinate', 'plan', 'organize'],
            CapabilityDomain.COORDINATION: ['coordinate', 'orchestrate', 'team', 'workflow']
        }
    ...
    def _calculate_improvement_trend(self, tasks: List[TaskResult]) -> float:
        ...
        # Sort tasks by date
        sorted_tasks = sorted(tasks, key=lambda t: t.completed_at if hasattr(t, 'completed_at') else datetime.now())
    ...
    def _identify_capability_patterns(self, profile: AgentCapabilityProfile) -> None:
        ...
        try:
            ...
            for domain, score in sorted_capabilities[:3]:
                if (score.proficiency_level.value >= 3 and 
                    score.confidence_score >= self.assessment_config['confidence_threshold']):
            ...
            for domain, score in sorted_capabilities[3:6]:
                if (score.proficiency_level.value >= 2 and 
            ...
            for domain, score in reversed(sorted_capabilities):
                if (score.evidence_count >= self.assessment_config['min_evidence_count'] and
                    score.proficiency_level.value <= 2):
    ...
    def _analyze_specializations(self, profile: AgentCapabilityProfile) -> None:
        ...
        try:
            ...
            high_proficiency_count = sum(
                1 for score in profile.capability_scores.values()
                if score.proficiency_level.value >= 3 and 
                   score.confidence_score >= self.assessment_config['confidence_threshold']
            )
            ...
            for domain, score in profile.capability_scores.items():
                if (score.proficiency_level.value >= 4 and 
                    score.confidence_score >= 0.8 and
    ...
    def _generate_development_recommendations(self, profile: AgentCapabilityProfile) -> None:
        ...
        try:
            ...
            for domain, score in profile.capability_scores.items():
                if (score.proficiency_level.value == 3 and 
                    score.improvement_trend > 0.1):
                    recommendations.append(f"Continue developing {domain.value} - showing strong improvement")
            
            # Versatility recommendations
            if profile.versatility_score < 0.3:
                recommendations.append("Consider expanding into new capability domains for increased versatility")
    ...
    def _get_task_types_for_domain(self, domain: CapabilityDomain) -> List[str]:
        ...
        domain_task_types = {
            CapabilityDomain.CODE_GENERATION: ["implementation", "feature_development", "bug_fix"],
            CapabilityDomain.CODE_REVIEW: ["code_review", "security_review", "performance_review"],
            CapabilityDomain.TESTING: ["unit_testing", "integration_testing", "test_automation"],
            CapabilityDomain.DOCUMENTATION: ["documentation", "api_docs", "user_guides"],
            CapabilityDomain.ARCHITECTURE: ["system_design", "architecture_review", "pattern_implementation"],
            CapabilityDomain.DEBUGGING: ["bug_investigation", "performance_debugging", "error_resolution"],
            CapabilityDomain.INTEGRATION: ["api_integration", "service_integration", "data_integration"],
            CapabilityDomain.PERFORMANCE_OPTIMIZATION: ["performance_tuning", "optimization", "profiling"],
            CapabilityDomain.SECURITY: ["security_audit", "vulnerability_assessment", "secure_coding"],
            CapabilityDomain.DATA_ANALYSIS: ["data_analysis", "reporting", "metrics_analysis"],
            CapabilityDomain.PROJECT_MANAGEMENT: ["project_planning", "task_coordination", "resource_management"],
            CapabilityDomain.COORDINATION: ["team_coordination", "workflow_management", "cross_team_collaboration"]
        }
    ...
    def get_capability_match_score(self, 
                                 agent_id: str,
                                 task_requirements: TaskCapabilityRequirement) -> float:
        """
        ...
        try:
            ...
            required_score = 0.0
            for domain, required_level in task_requirements.required_capabilities.items():
                if domain in profile.capability_scores:
                    agent_score = profile.capability_scores[domain]
                    level_match = min(1.0, agent_score.proficiency_level.value / required_level.value)
            ...
            if task_requirements.preferred_capabilities:
                for domain, preferred_level in task_requirements.preferred_capabilities.items():
                    if domain in profile.capability_scores:
                        agent_score = profile.capability_scores[domain]
                        level_match = min(1.0, agent_score.proficiency_level.value / preferred_level.value)
                        confidence_weight = agent_score.confidence_score
