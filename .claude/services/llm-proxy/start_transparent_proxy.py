...
def start_proxy():
    ...
    if config.get("AZURE_OPENAI_API_KEY"):
        ...
        env["OPENAI_BASE_URL"] = (
            f"{config['AZURE_OPENAI_ENDPOINT']}/openai/deployments/{config['AZURE_OPENAI_DEPLOYMENT_NAME']}"
        )
...
def launch_claude():
    ...
    print("   All LLM calls will be routed through the proxy\n")
...
"""
    ...
    print("   Add to your shell config: alias claude='claude-proxy'")
