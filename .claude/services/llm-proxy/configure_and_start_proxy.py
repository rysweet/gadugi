...
"""
    ...
    def test_connection(self):
        ...
        try:
            ...
            proxy_icon = (
                "✅"
                if health_data["proxy_status"] == "healthy"
                else "⚠️"
                if health_data["proxy_status"] == "degraded"
                else "❌"
            )
            ...
            upstream_icon = (
                ...
                if health_data["upstream_status"] in ["warning", "assumed-healthy"]
                else "❌"
            )
            ...
            if health_data["upstream_status"] in ["healthy", "assumed-healthy"]:
                ...
                print(
                    "   2. Restart the proxy: uv run python configure_and_start_proxy.py --stop && --start"
                )
                print(
                    "   3. Run this test again: uv run python configure_and_start_proxy.py --test"
                )

        except httpx.ConnectError:
            print("❌ Cannot connect to proxy at", PROXY_URL)
            print(
                "   Is the proxy running? Check with: uv run python configure_and_start_proxy.py --status"
            )
        except Exception as e:
