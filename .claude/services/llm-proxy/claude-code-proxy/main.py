...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("llm-proxy.log"), logging.StreamHandler()],
)
...
class LogIncomingRequestsMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        if request.method == "POST" and request.url.path == "/v1/messages":
            ...
            logger.info("[PROXY-LOG] Incoming /v1/messages POST request")
            ...
            try:
                ...
                logger.info(
                    f"[PROXY-LOG] Max tokens: {body_json.get('max_tokens', 'not specified')}"
                )
