...
class EventSubscriptionManager:
    ...
    def _initialize_default_subscriptions(self):
        ...
        self.add_subscription(
            "orchestration",
            "*.hasQuestion",
            "route_question_to_user",
            EventPriority.HIGH,
        )
        self.add_subscription(
            "orchestration",
            "*.needsApproval",
            "route_approval_to_user",
            EventPriority.HIGH,
        )
        self.add_subscription("orchestration", "*.stopped", "aggregate_results")
        self.add_subscription(
            "orchestration", "teamcoach.reflection_complete", "learn_from_session"
        )
        self.add_subscription(
            "orchestration", "decomposition.completed", "begin_execution_planning"
        )
        ...
        self.add_subscription(
            "taskdecomposer", "orchestration.problem_identified", "begin_decomposition"
        )
        self.add_subscription(
            "taskdecomposer", "orchestration.task_analyzed", "begin_decomposition"
        )
        self.add_subscription("taskdecomposer", "workflow.blocked", "analyze_blockage")
        self.add_subscription(
            "taskdecomposer", "decomposition.pattern_matched", "apply_learned_pattern"
        )
    ...
    def remove_subscription(self, agent_id: str, pattern: str) -> bool:
        ...
        if agent_id in self.subscriptions:
            ...
            self.subscriptions[agent_id] = [
                sub for sub in self.subscriptions[agent_id] if sub.pattern != pattern
            ]
    ...
    def get_subscription_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_priority_count": sum(
                ...
                for sub in subs
                if sub.priority in [EventPriority.HIGH, EventPriority.CRITICAL]
            ),
        }
