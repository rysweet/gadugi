...
class AgentEvent(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Type of event (supports v0.3 dynamic event types)"
    )
    ...
    stored_in_memory: bool = Field(
        default=False, description="Whether event is persisted"
    )
...
class TaskCompletedEvent(AgentEvent):
    ...
    success_metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Success metrics"
    )
...
class KnowledgeLearnedEvent(AgentEvent):
    ...
    confidence: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Confidence in knowledge"
    )
...
class CollaborationMessageEvent(AgentEvent):
    ...
    message_type: str = Field(
        ..., description="Message type (request, response, notification)"
    )
...
class EventReplayRequest(BaseModel):
    ...
    agent_id: Optional[str] = Field(
        None, description="Specific agent to replay (optional)"
    )
    from_timestamp: Optional[datetime] = Field(
        None, description="Replay from this timestamp"
    )
    to_timestamp: Optional[datetime] = Field(
        None, description="Replay until this timestamp"
    )
    event_types: Optional[List[EventType]] = Field(
        None, description="Event types to replay"
    )
...
class EventStorageInfo(BaseModel):
    ...
    oldest_event: Optional[datetime] = Field(
        None, description="Timestamp of oldest event"
    )
    newest_event: Optional[datetime] = Field(
