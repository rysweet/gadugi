    ...
    try:
        ...
        if memory_storage:
            try:
                ...
                health_info["memory_system"] = json.dumps(
                    memory_status
                )  # Convert to string
            except Exception as e:
                health_info["memory_system"] = json.dumps(
...
def list_events() -> Tuple[Any, int]:
    ...
    try:
        ...
        event_filter = EventFilter(
            event_types=[
                EventType(t) for t in params.get("event_types", "").split(",") if t
            ]
            or None,
            agent_ids=params.get("agent_ids", "").split(",")
            if params.get("agent_ids")
            else None,
            task_ids=params.get("task_ids", "").split(",")
            if params.get("task_ids")
            else None,
            project_ids=params.get("project_ids", "").split(",")
            if params.get("project_ids")
            else None,
            priority=EventPriority(params.get("priority"))
...
def filter_events() -> Tuple[Any, int]:
    ...
    try:
        ...
        # Apply filtering
        events = run_async_in_thread(
            filter_engine.filter_events(memory_storage, event_filter)
        )
