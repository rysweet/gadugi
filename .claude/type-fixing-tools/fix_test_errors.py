...
def fix_test_files():
    ...
    for filepath in test_files:
        ...
        if '"Any" is not defined' in content or "Any" in content:
            if (
                ...
                content = re.sub(
                    ...
                    lambda m: f"from typing import Any, {m.group(1)}"
        ...
        patterns = [
            ...
            (
                r"AgentInitializedEvent\(\s*agent_id=",
                'AgentInitializedEvent(agent_type="test", agent_id=',
            ),
            (
                r"TaskStartedEvent\(\s*agent_id=",
                'TaskStartedEvent(agent_type="test", agent_id=',
            ),
            (
                r"TaskCompletedEvent\(\s*agent_id=",
                'TaskCompletedEvent(agent_type="test", agent_id=',
            ),
            (
                r"KnowledgeLearnedEvent\(\s*agent_id=",
                'KnowledgeLearnedEvent(agent_type="test", agent_id=',
            ),
        ]
        ...
        content = re.sub(
            r"(mock_\w+\.assert_called.*)", r"\1  # type: ignore[attr-defined]", content
        )

        # Fix ProcessRegistry._get_current_time access
        content = re.sub(r"registry\._get_current_time\(\)", r"time.time()", content)
