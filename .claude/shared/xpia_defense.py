...
class ContentSanitizer:
    ...
    def sanitize_content(self, content: str, threats: List[Dict[str, Any]]) -> Tuple[str, List[str]]:
        """
        ...
        for threat in threats:
            ...
            if threat_level in ['critical', 'malicious']:
                ...
                sanitized, action = self._neutralize_threat(sanitized, match_text, pattern_name)
                if action:
                    actions.append(action)
            elif threat_level == 'suspicious':
                # Comment out or mark suspicious content
                sanitized, action = self._mark_suspicious(sanitized, match_text, pattern_name)
                if action:
                    actions.append(action)
        
        return sanitized, actions
    
    def _neutralize_threat(self, content: str, match_text: str, pattern_name: str) -> Tuple[str, Optional[str]]:
        ...
        if sanitized != content:
            return sanitized, f"Neutralized {pattern_name}: replaced '{match_text[:50]}...' with placeholder"
        
        return content, None
    
    def _mark_suspicious(self, content: str, match_text: str, pattern_name: str) -> Tuple[str, Optional[str]]:
        ...
        if marked_content != content:
            return marked_content, f"Marked suspicious {pattern_name}: '{match_text[:50]}...'"
            ...
            if len(match) > 8:  # Ignore short matches that might be false positives
                try:
                    ...
                    if decoded and decoded != match:
                        decoded_content = decoded_content.replace(match, f"[BASE64_DECODED: {decoded}]")
...
class XPIADefenseEngine:
    ...
    def validate_content(self, content: str, context: str = "general", 
                        strict_mode: Optional[bool] = None) -> ValidationResult:
        """
        ...
        sanitized_content, sanitization_actions = self.sanitizer.sanitize_content(
            decoded_content if decoded_content != content else content, 
            threats
        )
...
class XPIADefenseAgent:
    ...
    def validate_agent_input(self, content: str, agent_name: str = "unknown") -> ValidationResult:
        ...
        return self.engine.validate_content(
            content, 
            context=f"agent_input:{agent_name}"
        )
    ...
    def validate_file_content(self, content: str, filename: str = "unknown") -> ValidationResult:
        ...
        return self.engine.validate_content(
            content, 
            context=f"file_content:{filename}"
        )
    
    def validate_agent_communication(self, content: str, source_agent: str = "unknown", 
                                   target_agent: str = "unknown") -> ValidationResult:
        """Validate inter-agent communication"""
        return self.engine.validate_content(
            content, 
            context=f"agent_comm:{source_agent}->{target_agent}"
        )
