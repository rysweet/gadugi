...
class WorkflowValidator:
    ...
    def _validate_prompt_format(self, context: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            passed = len(issues) == 0
            message = (
                "Prompt format validation passed"
                if passed
                else f'Format issues found: {"; ".join(issues)}'
            )
        ...
        for i, phase in enumerate(completed_phases):
            if i < len(expected_sequence):
                if phase != expected_sequence[i]:
                    issues.append(
                        f"Expected phase {expected_sequence[i].name} at position {i}, got {phase.name}"
                    )
            else:
                # Phase beyond expected sequence
                issues.append(
                    f"Unexpected phase {phase.name} at position {i} (beyond expected sequence)"
                )
    ...
    def _generate_report(
        self, results: List[ValidationResult], start_time: datetime, report_type: str
    ) -> ValidationReport:
...
def validate_prompt(
    prompt_file: str, level: ValidationLevel = ValidationLevel.STANDARD
) -> ValidationReport:
...
def validate_workflow(
    prompt_file: str, workflow_state, level: ValidationLevel = ValidationLevel.STANDARD
) -> ValidationReport:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Error: Invalid validation level '{level_name}'. Use: minimal, standard, strict, comprehensive"
        )
        sys.exit(1)
