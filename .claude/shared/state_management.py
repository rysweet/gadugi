...
class StateManager:
    ...
    def save_state(self, *args, **kwargs) -> bool:
        """
        Save task state to persistent storage.
        Supports:
          - save_state(state: TaskState)
          - save_state(state_id: str, state_data: dict) [for test compatibility]
        """
        if len(args) == 1 and isinstance(args[0], TaskState):
            state = args[0]
            ...
            state_id, state_data = args
            data = dict(state_data)
            # Map 'phase' to 'current_phase_name' if present
            if "phase" in data:
                data["current_phase_name"] = data.pop("phase")
            # Map 'metadata' to 'context' if present
            if "metadata" in data:
                data["context"] = data.pop("metadata")
            # Remove any keys not in TaskState fields
            allowed = {
                "task_id",
                "prompt_file",
                "status",
            ...
            filtered["task_id"] = state_id
            if "prompt_file" not in filtered:
                filtered["prompt_file"] = "test-prompt.txt"
            if "status" not in filtered:
                filtered["status"] = "pending"
            state = TaskState.from_dict(filtered)
            return self.save_state(state)
        else:
            raise TypeError(
                "save_state() expects (TaskState) or (state_id: str, state_data: dict)"
            )
...
class CheckpointManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.checkpoint_dir = Path(
            self.config.get("checkpoint_dir", ".github/workflow-checkpoints")
        )
