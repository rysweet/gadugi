...
class SecurityAwareAgent(BaseAgent):
    ...
    def validate_input(self, input_data: Any) -> bool:
        ...
        if isinstance(input_data, str):
            dangerous_patterns = self.security_policies.get('dangerous_patterns', [
                '<script', 'javascript:', 'onerror=', 'onclick=',
                '; DROP TABLE', 'UNION SELECT', '../..', '\\x00'
            ])
            for pattern in dangerous_patterns:
                if pattern.lower() in input_data.lower():
                    self.audit_security_event('dangerous_pattern_detected', {
                        'pattern': pattern,
    ...
    def enforce_policy(self, action: str, context: Dict[str, Any]) -> bool:
        ...
        if allowed_actions and action not in allowed_actions:
            self.audit_security_event('policy_violation', {
                'action': action,
                'context': context
            })
            return False
    ...
    def stop_monitoring(self):
        ...
        if self.start_time:
            ...
            if execution_time > self.performance_thresholds['max_execution_time']:
                self.log_info(f"Performance warning: Execution time {execution_time}s exceeds threshold")
