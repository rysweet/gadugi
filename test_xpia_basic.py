...
from xpia_defense import XPIADefenseAgent, SecurityMode, ThreatLevel
...
def test_basic_functionality():
    ...
    print(f"  Threat level: {result.threat_level.value}")
    print(f"  Sanitized: {'Yes' if result.sanitized_content != result.original_content else 'No'}")
    print(f"  Processing time: {result.processing_time_ms:.2f}ms")
    
    # Test 4: Performance check
    print(f"\nPerformance Summary:")
    status = agent.get_security_status()
    print(f"  Average processing time: {status['performance_stats']['average_processing_time_ms']:.2f}ms")
