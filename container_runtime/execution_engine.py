...
from .image_manager import ImageManager, BuildContext

# Import Enhanced Separation shared modules
import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), '..', '.claude', 'shared', 'utils'))
from error_handling import GadugiError, RecoverableError, retry_on_failure
...
class ContainerExecutionEngine:
    ...
    def _build_container_config(self, request: ExecutionRequest, 
                               policy: ExecutionPolicy, image_name: str) -> ContainerConfig:
    ...
    def _execute_container(self, request_id: str, config: ContainerConfig, 
                          user_id: Optional[str]) -> ContainerResult:
        """Execute container with monitoring and audit logging."""
        
        # Log container creation
        create_event_id = self.audit_logger.log_container_created(
            container_id=request_id,
            image=config.image,
        ...
        try:
            ...
            if hasattr(result, 'container_id') and result.container_id in self.container_manager.active_containers:
    ...
    def execute_python_code(self, code: str, packages: Optional[List[str]] = None,
                           security_policy: Optional[str] = None,
                           timeout: Optional[int] = None,
                           user_id: Optional[str] = None) -> ExecutionResponse:
        """
    ...
    def execute_shell_script(self, script: str, security_policy: Optional[str] = None,
                            timeout: Optional[int] = None,
                            user_id: Optional[str] = None) -> ExecutionResponse:
        """
    ...
    def execute_node_code(self, code: str, packages: Optional[List[str]] = None,
                         security_policy: Optional[str] = None,
                         timeout: Optional[int] = None,
                         user_id: Optional[str] = None) -> ExecutionResponse:
        """
...
def execute_python(code: str, packages: Optional[List[str]] = None,
                  security_policy: str = "standard",
                  timeout: int = 300) -> ExecutionResponse:
...
def execute_shell(script: str, security_policy: str = "standard",
                 timeout: int = 300) -> ExecutionResponse:
...
def execute_node(code: str, packages: Optional[List[str]] = None,
                security_policy: str = "standard",
                timeout: int = 300) -> ExecutionResponse:
