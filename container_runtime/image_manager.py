                ...
                for name, data in registry_data.items():
                    self.image_registry[name] = ImageInfo(
                        ...
                        security_scan_date=datetime.fromisoformat(data['security_scan_date']) 
                                         if data.get('security_scan_date') else None,
            ...
            for name, info in self.image_registry.items():
                registry_data[name] = {
                    ...
                    'security_scan_date': info.security_scan_date.isoformat() 
                                        if info.security_scan_date else None,
...
"""
    ...
    def _register_image(self, image, runtime: str) -> None:
        ...
        try:
            ...
            image_info = ImageInfo(
                ...
                tag=image.tags[0].split(':')[1] if image.tags and ':' in image.tags[0] else 'latest',
                image_id=image.id,
                size=image.attrs.get('Size', 0),
                created=datetime.fromisoformat(image.attrs.get('Created', '').replace('Z', '+00:00')),
    ...
    def get_or_create_runtime_image(self, runtime: str, 
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        scanned_images = sum(1 for img in self.image_registry.values() 
                           if img.security_scan_date is not None)
        
        vulnerability_counts = [img.vulnerability_count for img in self.image_registry.values() 
                              if img.vulnerability_count is not None]
        
        security_scores = [img.security_score for img in self.image_registry.values() 
