...
class SecurityPolicyEngine:
    ...
    def _load_builtin_policies(self) -> None:
        ...
        standard_policy = ExecutionPolicy(
            ...
            security_constraints=SecurityConstraints(),
            allowed_images={
                "python:3.11-slim", "node:18-alpine", "alpine:latest",
                "ubuntu:22.04", "debian:11-slim"
            },
            blocked_commands={
                "sudo", "su", "passwd", "chown", "chmod", "mount", "umount",
                "iptables", "systemctl", "service", "crontab", "at"
            }
        )
        ...
        hardened_policy = ExecutionPolicy(
            ...
            blocked_commands={
                "sudo", "su", "passwd", "chown", "chmod", "mount", "umount",
                "iptables", "systemctl", "service", "crontab", "at", "wget",
                "curl", "nc", "netcat", "telnet", "ssh", "scp", "rsync"
            },
        ...
        for policy in [minimal_policy, standard_policy, hardened_policy, paranoid_policy]:
    ...
    def validate_execution_request(self, image: str, command: List[str], 
    ...
    def apply_policy_to_container_config(self, base_config: Dict[str, Any],
        ...
        config['user'] = f"{policy.security_constraints.user_id}:{policy.security_constraints.group_id}"
        ...
        ulimits.extend([
            {'Name': 'nproc', 'Soft': policy.resource_limits.processes, 
             'Hard': policy.resource_limits.processes},
            {'Name': 'nofile', 'Soft': policy.resource_limits.open_files,
        ...
        if policy.environment_whitelist:
            ...
            config['environment'] = {
                ...
                if k in policy.environment_whitelist
            }
