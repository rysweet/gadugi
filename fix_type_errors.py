...
def fix_unused_imports(content: str) -> str:
    ...
    for line in lines:
        ...
        if 'import' in line and any(unused in line for unused in [
            'import os', 'import json', 'import Path', 'import timedelta',
            'import Tuple', 'import Union', 'import MagicMock'
        ]):
...
def fix_possibly_unbound(content: str) -> str:
    ...
    if 'WorkflowStage' in content and 'from workflow_stages import WorkflowStage' not in content:
    ...
    if 'HealthStatus' in content and 'class HealthStatus' not in content:
        ...
        imports_to_add.append('class HealthStatus(Enum):\n    HEALTHY = "healthy"\n    UNHEALTHY = "unhealthy"')
