...
def create_cleanup_report():
    ...
    cleanup_targets = {
        ...
        "backup_created": [
            repo_path / ".github" / "Memory.md.backup"
        ]
    }
    ...
    for dir_path in cleanup_targets["directories"]:
        if dir_path.exists():
            try:
                ...
                print(f"  {'Would remove' if dry_run else 'Removed'} directory: {dir_path}")
...
def main():
    ...
    parser.add_argument("--report-only", action="store_true", help="Generate report without cleanup")
    parser.add_argument("--yes", "-y", action="store_true", help="Automatically confirm cleanup")
    ...
    print("=" * 50)
    print(f"Old system size: {report['old_system_components']['total_size_bytes'] / 1024:.1f} KB")
    print(f"New system status: {report['new_system_status']['memory_files_count']} memory files")
    print()
    
    if report["old_system_components"]["directories"] or report["old_system_components"]["files"]:
        print("Components to remove:")
        for dir_info in report["old_system_components"]["directories"]:
            if dir_info["exists"]:
                print(f"  Directory: {dir_info['path']} ({dir_info['file_count']} files)")
        for file_info in report["old_system_components"]["files"]:
            if file_info["exists"]:
                print(f"  File: {file_info['path']} ({file_info['size_bytes'] / 1024:.1f} KB)")
        print()
