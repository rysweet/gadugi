...
class AgentMemoryWrapper:
    ...
    def get_workflow_practices(self) -> List[str]:
        ...
        for section_name, section in workflow.get("sections", {}).items():
            if hasattr(section, 'entries'):
                practices.extend([f"{section_name}: {entry.content}" for entry in section.entries])
        return practices
    ...
    def record_error(self, error_type: str, description: str, solution: Optional[str] = None) -> bool:
